...
async function main() {
  ...
  await db.authSession.updateMany({ where: { userId: admin.id, revokedAt: null }, data: { revokedAt: new Date() } })
  ...
  console.log(`Password reset verified for ${admin.phone.slice(0, 4)}***${admin.phone.slice(-2)} (${updated.role}). A password change is required after login.`)
}
