...
function requiredAdminConfig() {
  ...
  if (!/^\+2507[2389]\d{7}$/.test(phone)) throw new Error('ADMIN_PHONE must be a private Rwanda number in +2507XXXXXXXX format')
  ...
  return { phone, password, email, name }
}

async function seedOwnerAdmin() {
  const admin = requiredAdminConfig()
  const existing = await prisma.user.findUnique({ where: { phone: admin.phone } })
  if (existing) {
    if (!['SUPER_ADMIN', 'ADMIN'].includes(existing.role)) throw new Error('ADMIN_PHONE already belongs to a non-admin account')
    // Never reset an existing owner's password, MFA, role, or session state during a data seed.
    return
  }
