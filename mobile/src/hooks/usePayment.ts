/**
 * usePayment — Mobile payment hook for MTN MoMo + Airtel Money.
 *
 * Same API as the web version (src/hooks/usePayment.ts) but adapted for React Native.
 *
 * Features:
 *   - initiateMoMoPayment(phone, amount, orderId) → starts payment
 *   - pollPaymentStatus(transactionRef) → polls every 5s, stops after 5min
 *   - handlePaymentSuccess() → called when payment confirmed
 *   - handlePaymentFailure() → called when payment fails
 *   - cancelPayment() → stops polling
 *
 * Usage:
 *   const { status, initiate, cancel } = usePayment()
    ...
    async (params: InitiateParams): Promise<boolean> => {
      ...
      try {
        const res = await fetch(`${API_BASE}/payments/momo`, {
          ...
          body: JSON.stringify({ orderId, phone, network }),
        })
        ...
        const poll = async () => {
          ...
          try {
            const statusRes = await fetch(
              `${API_BASE}/payments/status/${transactionRefRef.current}`
            )
            ...
            if (statusData.status === "PAID") {
              ...
              // Trigger local push notification on success
              if (Platform.OS !== "web") {
                try {
                  ...
                  await Notifications.scheduleNotificationAsync({
                    content: {
                      ...
                      body: `Your payment of ${params.amount} RWF has been received.`,
                      sound: true,
                    },
