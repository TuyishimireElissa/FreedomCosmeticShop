...
interface UsePaymentPollingResult {
  ...
  error: string | null
  /** Start polling */
  start: (paymentId: string, orderAccessToken: string) => void
  /** Stop polling */
  stop: () => void
  /** Reset to idle */
  reset: () => void
...
export function usePaymentPolling(): UsePaymentPollingResult {
  ...
  const orderAccessTokenRef = useRef<string | null>(null)
  ...
  const reset = useCallback(() => {
    ...
    orderAccessTokenRef.current = null
  }, [stop])

  const start = useCallback(
    (paymentId: string, orderAccessToken: string) => {
      // Stop any existing polling
      stop()
      paymentIdRef.current = paymentId
      orderAccessTokenRef.current = orderAccessToken
      setStatus("polling")
      setError(null)
      setElapsed(0)
      ...
      const poll = async () => {
        ...
        try {
          const res = await fetch(
            ...
            { headers: orderAccessTokenRef.current ? { 'X-Order-Access-Token': orderAccessTokenRef.current } : {} },
          )
