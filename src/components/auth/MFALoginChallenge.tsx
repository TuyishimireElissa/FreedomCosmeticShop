...
export default function MFALoginChallenge({
  ...
  return (
    <form onSubmit={submit} className="space-y-5">
      ...
      <button type="submit" disabled={loading} className="flex min-h-12 w-full items-center justify-center gap-2 rounded-xl bg-fcs-brand-strong text-base font-black text-white disabled:opacity-50">{loading ? <><Loader2 className="h-4 w-4 animate-spin" />{t('auth.otp_verifying')}</> : t('auth.verify_continue')}</button>
