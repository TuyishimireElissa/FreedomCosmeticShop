  ...
  return (
    <aside className="fixed inset-x-3 bottom-[calc(5rem+env(safe-area-inset-bottom))] z-[80] mx-auto max-w-xl rounded-2xl border border-gray-200 bg-white p-4 shadow-2xl md:bottom-5" aria-label={t('analytics_consent.title')}>
      <div className="flex items-start gap-3">
        <ShieldCheck className="mt-0.5 h-5 w-5 shrink-0 text-fcs-brand-text" aria-hidden="true" />
        <div>
      ...
      <div className="mt-4 grid grid-cols-2 gap-3">
        ...
        <button type="button" onClick={() => choose(true)} className="min-h-12 rounded-xl bg-fcs-brand-strong px-4 text-sm font-bold text-white">{t('analytics_consent.accept')}</button>
      </div>
    </aside>
  )
}
...
export function AnalyticsConsentSettings() {
  ...
  return (
    <section className="rounded-2xl border border-gray-200 bg-white p-5" aria-labelledby="analytics-consent-heading">
      <div className="flex items-start gap-3">
        <BarChart3 className="mt-0.5 h-5 w-5 shrink-0 text-fcs-brand-text" aria-hidden="true" />
        <div>
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-2">
        <button type="button" aria-pressed={choice === 'denied'} onClick={() => choose(false)} className={`min-h-12 rounded-xl border px-4 text-sm font-bold ${choice === 'denied' ? 'border-fcs-brand bg-rose-50 text-fcs-brand-text' : 'border-gray-300 text-gray-800'}`}>{t('analytics_consent.off')}</button>
        <button type="button" aria-pressed={choice === 'granted'} onClick={() => choose(true)} className={`min-h-12 rounded-xl border px-4 text-sm font-bold ${choice === 'granted' ? 'border-fcs-brand bg-rose-50 text-fcs-brand-text' : 'border-gray-300 text-gray-800'}`}>{t('analytics_consent.on')}</button>
      </div>
