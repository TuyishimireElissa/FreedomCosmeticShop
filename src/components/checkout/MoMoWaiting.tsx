...
export default function MoMoWaiting({ network, phone, total, status, remaining, onRetry, onCancel }: MoMoWaitingProps) {
  ...
  if (!waiting) return <div className="mt-4 rounded-2xl border border-red-200 bg-red-50 p-5 text-center"><span className="mx-auto grid h-14 w-14 place-items-center rounded-full bg-white text-red-500"><AlertCircle className="h-7 w-7" /></span><h3 className="mt-3 font-black text-gray-900">{status === 'timeout' ? t('checkout.payment_timeout_title') : t('checkout.payment_failed_title')}</h3><p className="mt-2 text-sm leading-6 text-gray-600">{status === 'timeout' ? t('checkout.payment_timeout_safe_help') : t('checkout.payment_failed_recovery')}</p><div className="mt-4 grid gap-2 sm:grid-cols-2"><button type="button" onClick={onRetry} className="flex min-h-12 items-center justify-center gap-2 rounded-xl bg-fcs-brand-strong font-black text-white"><RefreshCw className="h-4 w-4" />{t('checkout.retry_payment')}</button>{WA_CONFIG.isNumberConfigured && <a href={buildWhatsAppUrl(t('checkout.payment_help_message'))} onClick={() => trackWhatsAppClick('payment_help', { language: language === 'en' ? 'en' : 'rw', pagePath: '/checkout' })} target="_blank" rel="noreferrer" className="flex min-h-12 items-center justify-center rounded-xl border border-green-200 bg-white font-bold text-green-700">{t('checkout.get_whatsapp_help')}</a>}</div></div>

  return <div className="mt-4 rounded-2xl border-2 border-[#B76E79]/20 bg-white p-5 text-center shadow-sm">
    <div className="relative mx-auto h-24 w-24"><span className="absolute inset-0 animate-ping rounded-full bg-[#B76E79]/10" /><span className="relative mx-auto grid h-24 w-24 place-items-center rounded-full bg-fcs-brand-strong text-white shadow-lg shadow-rose-200"><Phone className="h-10 w-10" /></span></div>
    ...
    <div className="mt-4 rounded-2xl bg-gray-50 p-4"><div className="flex justify-between gap-3 text-sm"><span className="text-gray-500">{t('checkout.amount_to_approve')}</span><strong className="text-lg text-fcs-brand-text">{total}</strong></div><div className="mt-2 flex justify-between gap-3 text-sm"><span className="text-gray-500">{t('checkout.charging_phone')}</span><strong>{displayPhone}</strong></div></div>
    ...
    <p className="mt-4 flex items-center justify-center gap-2 text-sm text-gray-500"><Loader2 className="h-4 w-4 animate-spin text-fcs-brand-text" />{t('checkout.waiting_with_time', { time: clock(remaining) })}</p>
