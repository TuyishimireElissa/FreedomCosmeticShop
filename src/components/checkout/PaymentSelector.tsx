...
function PaymentCard({ selected, onClick, icon: Icon, title, description, accent, badge, disabled = false }: { selected: boolean; onClick: () => void; icon: typeof Smartphone; title: string; description: string; accent: 'mtn' | 'airtel' | 'card' | 'cod'; badge?: string; disabled?: boolean }) {
  ...
  return <button type="button" onClick={onClick} disabled={disabled} className={`flex min-h-[72px] w-full touch-manipulation items-center gap-3 rounded-2xl border-2 p-4 text-left transition-all ${selected ? 'border-[#B76E79] bg-rose-50/50 shadow-sm' : 'border-gray-100 bg-white hover:border-rose-100'} disabled:cursor-not-allowed disabled:opacity-45`}><span className={`grid h-11 w-11 shrink-0 place-items-center rounded-xl ${colors[accent]}`}><Icon className="h-5 w-5" /></span><span className="min-w-0 flex-1"><span className="flex flex-wrap items-center gap-2"><span className="text-base font-black text-[#1a1a1a]">{title}</span>{badge && <span className="rounded-full bg-[#FFD200] px-2 py-0.5 text-[10px] font-black uppercase text-[#1a1a1a]">{badge}</span>}</span><span className="mt-0.5 block text-sm text-gray-500">{description}</span></span><span className={`grid h-5 w-5 shrink-0 place-items-center rounded-full border-2 ${selected ? 'border-[#B76E79]' : 'border-gray-300'}`}>{selected && <span className="h-2.5 w-2.5 rounded-full bg-[#B76E79]" />}</span></button>
}
