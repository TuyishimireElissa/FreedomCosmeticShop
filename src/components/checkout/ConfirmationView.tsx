...
import { buildWhatsAppShareUrl, buildWhatsAppUrl, trackWhatsAppClick, WA_CONFIG } from '@/lib/whatsapp-service'

export interface ConfirmedCheckoutOrder { id: string; orderNumber: string; total: number; items: CartItem[]; orderAccessToken?: string; confirmationDelivery?: { sms: string; email: string } }
...
export default function ConfirmationView({ order, address, paymentMethod }: { order: ConfirmedCheckoutOrder; address: CheckoutAddress; paymentMethod: CheckoutPaymentMethod }) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/orders/${encodeURIComponent(order.id)}/track`, {
      signal: controller.signal,
      cache: 'no-store',
      headers: order.orderAccessToken ? { 'X-Order-Access-Token': order.orderAccessToken } : {},
    })
      .then((response) => { if (!response.ok) throw new Error(); return response.json() })
      .then((result) => setTracked(result.order || null))
      .catch(() => {})
    return () => controller.abort()
  }, [order.id, order.orderAccessToken])
  ...
  const hasWhatsApp = WA_CONFIG.isNumberConfigured
  const whatsappSupport = hasWhatsApp ? buildWhatsAppUrl(supportMessage) : '#'
  ...
  return <main className="mx-auto max-w-2xl">
    <section className="rounded-xl border border-[#EEEEEE] bg-white p-5 shadow-[0_1px_3px_rgba(0,0,0,0.04)] sm:p-9">
      <header className="text-center"><span className="mx-auto grid h-20 w-20 place-items-center rounded-full bg-fcs-success text-white"><Check className="h-10 w-10" /></span><h1 className="mt-5 text-3xl font-black text-gray-900">{t('confirmation.title')}</h1><p className="mt-2 text-sm text-gray-500">{customerFirstName ? t('confirmation.thank_you', { name: customerFirstName }) : t('confirmation.received')}</p><div className="mx-auto mt-4 inline-flex items-center gap-2 rounded-full bg-gray-100 px-4 py-2"><Package className="h-4 w-4 text-fcs-brand-text" /><span className="text-xs text-gray-500">{t('confirmation.order_number_label')}</span><strong className="font-mono text-sm text-fcs-brand-text">{order.orderNumber}</strong></div></header>
      ...
      <div className="mt-6 grid gap-3 sm:grid-cols-2"><Link href="/track-order" className="flex min-h-[52px] items-center justify-center gap-2 rounded-[10px] bg-fcs-brand-strong font-black text-white"><Package className="h-5 w-5" />{t('confirmation.track_order')}</Link><a href={buildWhatsAppShareUrl(message)} onClick={() => trackWhatsAppClick('track_order', { language: language === 'en' ? 'en' : 'rw', pagePath: '/checkout' })} target="_blank" rel="noreferrer" className="flex min-h-[52px] items-center justify-center gap-2 rounded-[10px] bg-[#25D366] font-black text-white"><Share2 className="h-5 w-5" />{t('confirmation.share_whatsapp')}</a></div>
...
function InfoCard({ icon, title, className = '', children }: { icon: React.ReactNode; title: string; className?: string; children: React.ReactNode }) { return <section className={`rounded-xl border border-[#EEEEEE] bg-[#FAFAFA] p-4 ${className}`}><h2 className="flex items-center gap-2 text-sm font-black text-gray-900"><span className="text-fcs-brand-text">{icon}</span>{title}</h2><div className="mt-3">{children}</div></section> }
