...
import { Check, Clock, MapPin, MessageCircle, Package, Phone, Share2, ShoppingBag } from 'lucide-react'
...
export default function ConfirmationView({ order, address, paymentMethod }: { order: ConfirmedCheckoutOrder; address: CheckoutAddress; paymentMethod: CheckoutPaymentMethod }) {
  ...
  const whatsappSupport = getWhatsAppLink(supportMessage)
  const hasWhatsApp = whatsappSupport.startsWith('https://')
  const hasPhone = !BUSINESS.phone.includes('TODO:')

  return <main className="mx-auto max-w-2xl">
    <section className="rounded-[2rem] border border-emerald-100 bg-white p-5 shadow-xl shadow-black/5 sm:p-9">
      <header className="text-center"><span className="mx-auto grid h-20 w-20 place-items-center rounded-full bg-emerald-500 text-white shadow-lg shadow-emerald-200"><Check className="h-10 w-10" /></span><h1 className="mt-5 text-3xl font-black text-gray-900">{t('confirmation.title')}</h1><p className="mt-2 text-sm text-gray-500">{customerFirstName ? t('confirmation.thank_you', { name: customerFirstName }) : t('confirmation.received')}</p><div className="mx-auto mt-4 inline-flex items-center gap-2 rounded-full bg-gray-100 px-4 py-2"><Package className="h-4 w-4 text-[#B76E79]" /><span className="text-xs text-gray-500">{t('confirmation.order_number_label')}</span><strong className="font-mono text-sm text-[#B76E79]">{order.orderNumber}</strong></div></header>

      <div className="mt-7 grid gap-4 sm:grid-cols-2">
        <InfoCard icon={<span aria-hidden>💳</span>} title={t('confirmation.payment_title')}><Row label={t('confirmation.payment_method')} value={paymentLabel} /><Row label={t('confirmation.payment_status')} value={paymentStatus} /><Row label={t('cart.total')} value={formatRWF(order.total)} strong /></InfoCard>
      ...
      <div className="mt-6 grid gap-3 sm:grid-cols-2"><Link href="/track-order" className="flex min-h-[52px] items-center justify-center gap-2 rounded-xl bg-[#B76E79] font-black text-white"><Package className="h-5 w-5" />{t('confirmation.track_order')}</Link><a href={`https://wa.me/?text=${encodeURIComponent(message)}`} target="_blank" rel="noreferrer" className="flex min-h-[52px] items-center justify-center gap-2 rounded-xl bg-[#25D366] font-black text-white"><Share2 className="h-5 w-5" />{t('confirmation.share_whatsapp')}</a></div>
      <Link href="/products" className="mt-3 flex min-h-12 w-full items-center justify-center gap-2 rounded-xl border-2 border-gray-200 font-bold text-gray-700"><ShoppingBag className="h-5 w-5" />{t('confirmation.continue_shopping')}</Link>

      <section className="mt-6 rounded-2xl bg-gray-50 p-4"><h2 className="font-black text-gray-900">{t('confirmation.support_title')}</h2><p className="mt-1 text-sm text-gray-500">{t('confirmation.support_hint')}</p><div className="mt-3 flex flex-col gap-2 sm:flex-row">{hasWhatsApp && <a href={whatsappSupport} target="_blank" rel="noreferrer" className="flex min-h-12 flex-1 items-center justify-center gap-2 rounded-xl bg-green-600 font-bold text-white"><MessageCircle className="h-4 w-4" />WhatsApp</a>}{hasPhone && <a href={`tel:${BUSINESS.phone}`} className="flex min-h-12 flex-1 items-center justify-center gap-2 rounded-xl border bg-white font-bold text-gray-700"><Phone className="h-4 w-4" />{t('confirmation.call_support')}</a>}<Link href="/contact" className="flex min-h-12 flex-1 items-center justify-center rounded-xl border bg-white font-bold text-gray-700">{t('confirmation.contact_support')}</Link></div>{!hasWhatsApp && !hasPhone && <p className="mt-2 text-xs text-amber-700">{t('confirmation.direct_contact_unavailable')}</p>}</section>
    </section>
  </main>
}

function InfoCard({ icon, title, className = '', children }: { icon: React.ReactNode; title: string; className?: string; children: React.ReactNode }) { return <section className={`rounded-2xl border border-gray-100 bg-gray-50 p-4 ${className}`}><h2 className="flex items-center gap-2 text-sm font-black text-gray-900"><span className="text-[#B76E79]">{icon}</span>{title}</h2><div className="mt-3">{children}</div></section> }
