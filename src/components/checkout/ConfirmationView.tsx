...
export default function ConfirmationView({ order, address, paymentMethod }: { order: ConfirmedCheckoutOrder; address: CheckoutAddress; paymentMethod: CheckoutPaymentMethod }) {
  ...
  return <main className="mx-auto max-w-2xl">
    <section className="rounded-[2rem] border border-emerald-100 bg-white p-5 shadow-xl shadow-black/5 sm:p-9">
      ...
      <div className="mt-6 grid gap-3 sm:grid-cols-2"><Link href="/track-order" className="flex min-h-[52px] items-center justify-center gap-2 rounded-xl bg-[#B76E79] font-black text-white"><Package className="h-5 w-5" />{t('confirmation.track_order')}</Link><a href={buildWhatsAppShareUrl(message)} onClick={() => trackWhatsAppClick('track_order', { language: language === 'en' ? 'en' : 'rw', pagePath: '/checkout' })} target="_blank" rel="noreferrer" className="flex min-h-[52px] items-center justify-center gap-2 rounded-xl bg-[#25D366] font-black text-white"><Share2 className="h-5 w-5" />{t('confirmation.share_whatsapp')}</a></div>
