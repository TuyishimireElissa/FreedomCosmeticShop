...
export default function AddressForm({ value, onChange, errors = {} }: AddressFormProps) {
  ...
  return <div className="space-y-5">
    ...
    <div className="grid gap-3 sm:grid-cols-2">
      {user && <button type="button" onClick={saveAddress} disabled={saving || !value.village} className="flex min-h-12 items-center justify-center gap-2 rounded-xl border-2 border-rose-100 font-bold text-fcs-brand-text disabled:opacity-40">{saved ? <Check className="h-4 w-4" /> : <Save className="h-4 w-4" />}{saving ? t('common.loading') : saved ? t('checkout.address_saved') : t('checkout.save_address_button')}</button>}
