    ...
    <main className="min-h-screen bg-[#f8f9fa] px-4 py-10 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-6xl">
        <header className="max-w-3xl">
          <span className="inline-flex items-center gap-2 text-xs font-black uppercase tracking-[0.16em] text-fcs-brand-text"><BookOpen className="h-4 w-4" aria-hidden="true" />{t('blog.label')}</span>
            ...
            {posts.map((post) => {
              ...
              return (
                <article key={post.slug} className="flex min-h-64 flex-col rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
                  {post.category && <span className="text-xs font-bold uppercase tracking-wide text-fcs-brand-text">{post.category}</span>}
                  ...
                  <Link href={`/blog/${post.slug}`} className="mt-auto inline-flex min-h-12 items-center gap-2 pt-5 text-sm font-bold text-fcs-brand-text">{t('blog.read_more')}<ArrowRight className="h-4 w-4" aria-hidden="true" /></Link>
                </article>
              )
            })}
          </section>
