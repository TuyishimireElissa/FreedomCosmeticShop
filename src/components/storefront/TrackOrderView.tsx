...
export function TrackOrderView() {
  ...
  const handleTrack = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`/api/orders/${encodeURIComponent(orderNumber.trim())}/track`, { cache: 'no-store', headers: { 'X-Order-Phone': phone.trim() } })
      if (!res.ok) {
  ...
  useOrderUpdates((event, data) => {
    ...
    if (event.startsWith("order:") && event !== "order:new") {
      ...
      if (event === "order:shipped" || event === "order:delivered") {
        fetch(`/api/orders/${encodeURIComponent(order.orderNumber)}/track`, { cache: 'no-store', headers: { 'X-Order-Phone': phone.trim() } })
          .then((r) => r.json())
          .then((d) => {
            if (d.order) {
              setOrder(d.order)
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-8 sm:px-6 lg:px-8">
      ...
      {/* Search form */}
      <form onSubmit={handleTrack} className="mb-6 space-y-3">
        <div className="grid gap-3 sm:grid-cols-2">
          <label className="block">
            <span className="mb-1 block text-xs font-bold text-muted-foreground">{t('checkout.order_number')}</span>
            <span className="relative block">
              <Search className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" />
              <Input type="text" placeholder="e.g. FCS-2026-..." value={orderNumber} onChange={(e) => setOrderNumber(e.target.value.toUpperCase())} className="h-12 pl-9 font-mono" autoComplete="off" required />
            </span>
          </label>
          <label className="block">
            <span className="mb-1 block text-xs font-bold text-muted-foreground">{t('auth.phone')}</span>
            <span className="relative block">
              <Phone className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" />
              <Input type="tel" inputMode="tel" value={phone} onChange={(e) => setPhone(e.target.value)} className="h-12 pl-9" autoComplete="tel" required />
            </span>
          </label>
        </div>
