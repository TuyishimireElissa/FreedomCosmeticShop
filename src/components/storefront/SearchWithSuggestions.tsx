  ...
  return (
    <div ref={containerRef} className={`relative ${className}`} role="search">
      <div className="relative">
        {loading ? <Loader2 className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 animate-spin text-fcs-brand-text" /> : <Search className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-gray-400" />}
        <input
          ref={inputRef}
          type="search"
          value={query}
          ...
          className={`w-full rounded-2xl border-2 border-gray-200 bg-white pl-10 pr-11 text-base text-gray-900 outline-none transition-colors focus:border-fcs-brand ${variant === 'hero' || variant === 'page' ? 'min-h-[52px] py-3' : 'min-h-11 py-2'}`}
      ...
      {isOpen && (showEmptyMenu || showResults) && (
        <div id="search-suggestions-listbox" role="listbox" className="absolute left-0 right-0 top-full z-50 mt-2 max-h-[min(70vh,520px)] overflow-y-auto rounded-2xl border border-gray-100 bg-white shadow-xl">
          {showEmptyMenu && <div className="space-y-4 p-3">
            ...
            {popularSearches.length > 0 && <section><p className="mb-2 flex items-center gap-1.5 text-xs font-bold uppercase tracking-wider text-gray-500"><TrendingUp className="h-3 w-3" />{t('search.popular')}</p><div className="flex flex-wrap gap-2">{popularSearches.map((term) => <button key={term} type="button" onClick={() => { setQuery(term); navigateToSearch(term) }} className="min-h-9 rounded-full bg-rose-50 px-3 text-xs font-medium text-fcs-brand-text">{term}</button>)}</div></section>}
          ...
          {showResults && !loading && suggestions && <>
            ...
            {hasResults ? <div className="p-2">
              ...
              <button type="button" onClick={() => navigateToSearch(query)} className="mt-2 flex min-h-12 w-full items-center justify-center gap-2 border-t text-sm font-bold text-fcs-brand-text">{t('search.view_all_results', { query })}<ArrowRight className="h-4 w-4" /></button>
