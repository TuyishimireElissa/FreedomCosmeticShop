...
export default function CartView() {
  ...
  if (cart.items.length === 0 && cart.savedItems.length === 0) return <main className="mx-auto grid min-h-[65vh] max-w-2xl place-items-center px-4 py-16 text-center"><div><span className="mx-auto grid h-24 w-24 place-items-center rounded-full bg-rose-50"><ShoppingCart className="h-10 w-10 text-[#B76E79]" /></span><h1 className="mt-5 text-3xl font-bold">{t('empty.cart')}</h1><p className="mt-2 text-sm text-gray-500">{t('empty.cart_hint')}</p><Link href="/products" className="mt-6 inline-flex min-h-12 items-center rounded-[10px] bg-[#B76E79] px-6 font-bold text-white">{t('cart.browse_products')}</Link></div></main>

  return <main className="mx-auto max-w-6xl px-4 py-6"><h1 className="text-2xl font-bold">{t('cart.title')} <span className="text-sm font-medium text-gray-400">({t('cart.items', { count: cart.itemCount })})</span></h1>
    {cart.lastRemoved && Date.now() - cart.lastRemoved.timestamp <= 10_000 && <div className="fixed bottom-[calc(76px+env(safe-area-inset-bottom))] left-4 right-4 z-[70] flex items-center gap-3 rounded-2xl bg-gray-900 p-3 text-white shadow-xl sm:left-auto sm:w-96"><span className="min-w-0 flex-1 truncate text-sm">{t('cart.undo_remove', { name: cart.lastRemoved.item.name })}</span><button type="button" onClick={cart.undoRemove} className="min-h-11 rounded-xl bg-white px-4 text-sm font-bold text-[#B76E79]">{t('cart.undo')}</button></div>}
    <div className="mt-6 grid gap-6 lg:grid-cols-[1fr_360px]"><div className="space-y-4">
      {delivery && <section className="rounded-xl border border-[#EEEEEE] bg-white p-4"><p className="flex items-center gap-2 text-sm font-bold"><Truck className="h-4 w-4 text-[#B76E79]" />{delivery.isFreeDelivery ? t('cart.free_delivery_achieved') : t('cart.free_delivery_hint', { amount: formatRWF(delivery.amountNeededForFree) })}</p><div className="mt-3 h-2 overflow-hidden rounded-full bg-gray-100"><div className="h-full bg-[#B76E79]" style={{ width: `${Math.min(100, delivery.freeDeliveryThreshold > 0 ? cart.subtotal / delivery.freeDeliveryThreshold * 100 : 0)}%` }} /></div></section>}
      ...
      {crossSells.length > 0 && <section className="rounded-xl border border-[#EEEEEE] bg-white p-4"><h2 className="font-bold">{t('cart.complete_routine')}</h2><p className="mt-1 text-xs text-gray-500">{t('cart.same_category_suggestions')}</p><div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-3">{crossSells.map((product) => <article key={product.id} className="overflow-hidden rounded-xl border"><Link href={`/products/${product.slug}`} className="relative block aspect-square bg-gray-50">{product.imageUrl && <Image src={product.imageUrl} alt={product.imageAlt} fill sizes="(max-width: 640px) 45vw, 180px" className="object-contain p-2" />}</Link><div className="p-3"><Link href={`/products/${product.slug}`} className="line-clamp-2 text-xs font-bold">{product.name}</Link><p className="mt-2 text-sm font-bold text-[#B76E79]">{formatRWF(product.price)}</p><button type="button" onClick={() => cart.addToCart({ productId: product.id, name: product.name, slug: product.slug, price: isWholesale && product.wholesalePrice ? product.wholesalePrice : product.price, retailPrice: product.price, wholesalePrice: product.wholesalePrice || undefined, comparePrice: product.compareAt || undefined, quantity: 1, maxQuantity: product.stock, image: product.imageUrl || undefined, imagePublicId: product.imagePublicId || undefined, imageAlt: product.imageAlt, volume: product.volume || undefined, brandName: product.brandName || undefined, categorySlug: product.categorySlug })} className="mt-2 flex min-h-11 w-full items-center justify-center gap-1 rounded-xl bg-rose-50 px-2 text-xs font-bold text-[#B76E79]"><Plus className="h-4 w-4" />{t('product.add_to_cart')}</button></div></article>)}</div></section>}
    </div>
