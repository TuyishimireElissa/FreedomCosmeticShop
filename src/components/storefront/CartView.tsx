...
export default function CartView() {
  ...
  return <main className="mx-auto max-w-6xl px-4 py-6"><h1 className="text-2xl font-bold">{t('cart.title')} <span className="text-sm font-medium text-gray-400">({t('cart.items', { count: cart.itemCount })})</span></h1>
    ...
    <div className="mt-6 grid gap-6 lg:grid-cols-[1fr_360px]"><div className="space-y-4">
      ...
      {crossSells.length > 0 && <section className="rounded-xl border border-[#EEEEEE] bg-white p-4"><h2 className="font-bold">{t('cart.complete_routine')}</h2><p className="mt-1 text-xs text-gray-500">{t('cart.same_category_suggestions')}</p><div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-3">{crossSells.map((product) => <article key={product.id} className="overflow-hidden rounded-xl border"><Link href={`/products/${product.slug}`} className="relative block aspect-square bg-gray-50">{product.imageUrl && <Image src={product.imageUrl} alt={product.imageAlt} fill sizes="(max-width: 640px) 45vw, 180px" className="object-contain p-2" />}</Link><div className="p-3"><Link href={`/products/${product.slug}`} className="line-clamp-2 text-xs font-bold">{product.name}</Link><p className="mt-2 text-sm font-bold text-[#B76E79]">{formatRWF(product.price)}</p><button type="button" onClick={() => cart.addToCart({ productId: product.id, name: product.name, slug: product.slug, price: product.price, comparePrice: product.compareAt || undefined, quantity: 1, maxQuantity: product.stock, image: product.imageUrl || undefined, imagePublicId: product.imagePublicId || undefined, imageAlt: product.imageAlt, volume: product.volume || undefined, brandName: product.brandName || undefined, categorySlug: product.categorySlug })} className="mt-2 flex min-h-11 w-full items-center justify-center gap-1 rounded-xl bg-rose-50 px-2 text-xs font-bold text-[#B76E79]"><Plus className="h-4 w-4" />{t('product.add_to_cart')}</button></div></article>)}</div></section>}
    </div>
    ...
    {cart.items.length > 0 && <div className="h-24 md:hidden" />}<div className="fixed bottom-[calc(64px+env(safe-area-inset-bottom))] left-0 right-0 z-40 border-t border-[#EEEEEE] bg-white p-3 shadow-[0_-2px_12px_rgba(0,0,0,0.06)] md:hidden"><div className="flex items-center gap-3"><div className="min-w-0 flex-1"><p className="text-xs text-gray-500">{t('cart.items', { count: cart.itemCount })}</p><p className="text-lg font-bold text-[#B76E79]">{formatRWF(total)}</p></div><Link href="/checkout" className="flex min-h-[52px] items-center rounded-[10px] bg-[#B76E79] px-6 font-bold text-white">{t('cart.checkout')}</Link></div></div>
  </main>
}
...
function CartRow({ item, onRemove, onSave, onQuantity }: { item: CartItem; onRemove: () => void; onSave: () => void; onQuantity: (quantity: number) => void }) { const { t } = useLanguage(); const image = item.imagePublicId ? `https://res.cloudinary.com/dohoc0tmp/image/upload/w_300,h_300,c_fill,q_auto,f_auto/${item.imagePublicId}` : item.image; return <article className="rounded-xl border border-[#EEEEEE] bg-white p-4"><div className="flex gap-3"><Link href={item.isBundle ? `/bundles/${item.slug}` : `/products/${item.slug}`} className="relative h-20 w-20 shrink-0 overflow-hidden rounded-xl bg-gray-50">{image && <Image src={image} alt={item.imageAlt || item.name} fill sizes="80px" className="object-contain p-1" />}</Link><div className="min-w-0 flex-1"><h2 className="line-clamp-2 text-sm font-bold">{item.name}</h2>{item.volume && <p className="text-xs text-gray-400">{item.volume}</p>}<p className="mt-1 font-bold text-[#B76E79]">{formatRWF(item.price * item.quantity)}</p><p className="mt-1 text-xs text-gray-500">{t('cart.stock_available', { count: item.maxQuantity })}</p></div><IconButton label={`${t('cart.remove')}: ${item.name}`} icon={<Trash2 className="h-4 w-4" />} variant="danger" onClick={onRemove} /></div><div className="mt-3 flex items-center justify-between border-t pt-3"><div className="flex h-11 items-center overflow-hidden rounded-xl border"><IconButton label={`${t('product.decrease_quantity')}: ${item.name}`} icon={<Minus className="h-4 w-4" />} onClick={() => onQuantity(item.quantity - 1)} disabled={item.quantity <= 1} className="rounded-none" /><span className="grid h-11 min-w-11 place-items-center border-x text-sm font-bold">{item.quantity}</span><IconButton label={`${t('product.increase_quantity')}: ${item.name}`} icon={<Plus className="h-4 w-4" />} onClick={() => onQuantity(item.quantity + 1)} disabled={item.quantity >= item.maxQuantity} className="rounded-none" /></div><button type="button" onClick={onSave} className="flex min-h-11 items-center gap-1.5 px-2 text-xs font-bold text-gray-500"><Heart className="h-4 w-4" />{t('cart.save_for_later')}</button></div>{item.quantity >= item.maxQuantity && <p className="mt-2 text-xs font-semibold text-amber-700">{t('cart.max_reached')}</p>}</article> }
