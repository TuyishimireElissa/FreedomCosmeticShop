...
interface CrossSell { id: string; name: string; slug: string; price: number; wholesalePrice?: number | null; compareAt: number | null; stock: number; volume: string | null; categorySlug: string; brandName: string | null; imageUrl: string | null; imagePublicId: string | null; imageAlt: string }
...
export default function CartView() {
  ...
  const discount = isWholesale ? 0 : appliedCoupon?.discountAmount || 0
  const deliveryFee = isWholesale ? 0 : appliedCoupon?.freeShipping ? 0 : delivery?.fee || 0
  const total = isWholesale ? cart.subtotal : Math.max(0, cart.subtotal - discount + deliveryFee)
  ...
  if (cart.items.length === 0 && cart.savedItems.length === 0) return <main className="mx-auto grid min-h-[65vh] max-w-2xl place-items-center px-4 py-16 text-center"><div><span className="mx-auto grid h-24 w-24 place-items-center rounded-full bg-rose-50"><ShoppingCart className="h-10 w-10 text-fcs-brand-text" /></span><h1 className="mt-5 text-3xl font-bold">{t('empty.cart')}</h1><p className="mt-2 text-sm text-gray-500">{t('empty.cart_hint')}</p><Link href="/products" className="mt-6 inline-flex min-h-12 items-center rounded-[10px] bg-fcs-brand-strong px-6 font-bold text-white">{t('cart.browse_products')}</Link></div></main>

  return <main className="mx-auto max-w-6xl px-4 py-6"><h1 className="text-2xl font-bold">{t('cart.title')} <span className="text-sm font-medium text-fcs-text-muted">({t('cart.items', { count: cart.itemCount })})</span></h1>
    {cart.lastRemoved && Date.now() - cart.lastRemoved.timestamp <= 10_000 && <div className="fixed bottom-[calc(76px+env(safe-area-inset-bottom))] left-4 right-4 z-[70] flex items-center gap-3 rounded-2xl bg-gray-900 p-3 text-white shadow-xl sm:left-auto sm:w-96"><span className="min-w-0 flex-1 truncate text-sm">{t('cart.undo_remove', { name: cart.lastRemoved.item.name })}</span><button type="button" onClick={cart.undoRemove} className="min-h-11 rounded-xl bg-white px-4 text-sm font-bold text-fcs-brand-text">{t('cart.undo')}</button></div>}
    <div className="mt-6 grid gap-6 lg:grid-cols-[1fr_360px]"><div className="space-y-4">
      {delivery && <section className="rounded-xl border border-[#EEEEEE] bg-white p-4"><p className="flex items-center gap-2 text-sm font-bold"><Truck className="h-4 w-4 text-fcs-brand-text" />{delivery.isFreeDelivery ? t('cart.free_delivery_achieved') : t('cart.free_delivery_hint', { amount: formatRWF(delivery.amountNeededForFree) })}</p><div className="mt-3 h-2 overflow-hidden rounded-full bg-gray-100"><div className="h-full bg-fcs-brand" style={{ width: `${Math.min(100, delivery.freeDeliveryThreshold > 0 ? cart.subtotal / delivery.freeDeliveryThreshold * 100 : 0)}%` }} /></div></section>}
      {cart.items.map((item) => <SwipeToDelete key={item.productId} onDelete={() => cart.removeItem(item.productId)} deleteLabel={`${t('cart.remove')}: ${item.name}`}><CartRow item={item} isWholesale={isWholesale} onRemove={() => cart.removeItem(item.productId)} onSave={() => cart.saveForLater(item.productId)} onQuantity={(quantity) => cart.updateQuantity(item.productId, quantity)} /></SwipeToDelete>)}
      {cart.savedItems.length > 0 && <section className="rounded-xl border border-[#EEEEEE] bg-white p-4"><h2 className="font-bold">{t('cart.saved_for_later', { count: cart.savedItems.length })}</h2><div className="mt-3 space-y-2">{cart.savedItems.map((item) => <div key={item.productId} className="flex items-center gap-3 border-t pt-2"><span className="min-w-0 flex-1 truncate text-sm font-bold">{item.name}</span><button type="button" onClick={() => cart.moveToCart(item.productId)} className="min-h-11 px-3 text-xs font-bold text-fcs-brand-text">{t('cart.move_to_cart')}</button><IconButton label={`${t('cart.remove')}: ${item.name}`} icon={<X className="h-4 w-4" />} variant="danger" onClick={() => cart.removeSaved(item.productId)} /></div>)}</div></section>}
      <section className="rounded-xl border border-[#EEEEEE] bg-white p-4"><h2 className="flex items-center gap-2 font-bold"><Truck className="h-4 w-4 text-fcs-brand-text" />{t('cart.delivery_estimate')}</h2><select value={cart.selectedDistrict} onChange={(event) => cart.setDistrict(event.target.value)} className="mt-3 min-h-12 w-full rounded-xl border-2 border-gray-200 bg-white px-3 text-base"><option value="">{t('delivery.select_district')}</option>{districts.map((group) => <optgroup key={group.province} label={group.province}>{group.districts.map((district) => <option key={district}>{district}</option>)}</optgroup>)}</select>{delivery && <div className="mt-3 rounded-xl bg-gray-50 p-3 text-sm"><p className="flex justify-between"><span>{delivery.deliveryTime}</span><strong>{delivery.fee === 0 ? t('common.free') : formatRWF(delivery.fee)}</strong></p></div>}</section>
      ...
      {crossSells.length > 0 && <section className="rounded-xl border border-[#EEEEEE] bg-white p-4"><h2 className="font-bold">{t('cart.complete_routine')}</h2><p className="mt-1 text-xs text-gray-500">{t('cart.same_category_suggestions')}</p><div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-3">{crossSells.map((product) => <article key={product.id} className="overflow-hidden rounded-xl border"><Link href={`/products/${product.slug}`} className="relative block aspect-square bg-gray-50">{product.imageUrl && <Image src={product.imageUrl} alt={product.imageAlt} fill sizes="(max-width: 640px) 45vw, 180px" className="object-contain p-2" />}</Link><div className="p-3"><Link href={`/products/${product.slug}`} className="line-clamp-2 text-xs font-bold">{product.name}</Link><p className="mt-2 text-sm font-bold text-fcs-brand-text">{formatRWF(product.price)}</p><button type="button" onClick={() => cart.addToCart({ productId: product.id, name: product.name, slug: product.slug, price: isWholesale && product.wholesalePrice ? product.wholesalePrice : product.price, retailPrice: product.price, wholesalePrice: product.wholesalePrice || undefined, comparePrice: product.compareAt || undefined, quantity: 1, maxQuantity: product.stock, image: product.imageUrl || undefined, imagePublicId: product.imagePublicId || undefined, imageAlt: product.imageAlt, volume: product.volume || undefined, brandName: product.brandName || undefined, categorySlug: product.categorySlug })} className="mt-2 flex min-h-11 w-full items-center justify-center gap-1 rounded-xl bg-rose-50 px-2 text-xs font-bold text-fcs-brand-text"><Plus className="h-4 w-4" />{t('product.add_to_cart')}</button></div></article>)}</div></section>}
    </div>
    <aside className="rounded-xl border border-[#EEEEEE] bg-white p-5 lg:sticky lg:top-36"><h2 className="font-bold">{t('cart.total')}</h2><Summary label={t('cart.subtotal')} value={formatRWF(cart.subtotal)} />{discount > 0 && <Summary label={t('cart.discount')} value={`−${formatRWF(discount)}`} green />}{cart.selectedDistrict && <Summary label={t('cart.delivery')} value={deliveryFee === 0 ? t('common.free') : formatRWF(deliveryFee)} />}<div className="mt-4 border-t pt-4"><Summary label={t('cart.total')} value={formatRWF(total)} strong /></div>
      {!isWholesale && (!appliedCoupon ? <div className="mt-5 border-t pt-4"><div className="flex gap-2"><input value={couponCode} onChange={(e) => { setCouponCode(e.target.value.toUpperCase()); setCouponPreview(null); setCouponError('') }} placeholder={t('cart.coupon_placeholder')} className="min-h-11 min-w-0 flex-1 rounded-xl border px-3" /><button type="button" onClick={previewCoupon} disabled={couponLoading} className="min-h-11 rounded-xl bg-gray-900 px-4 text-sm font-bold text-white">{couponLoading ? t('cart.applying') : t('cart.apply_coupon')}</button></div>{couponError && <p className="mt-2 flex gap-2 rounded-xl bg-red-50 p-3 text-xs text-red-700"><AlertCircle className="h-4 w-4 shrink-0" />{couponError}</p>}{couponPreview && <div className="mt-3 rounded-xl border border-amber-200 bg-amber-50 p-3"><h3 className="font-bold text-amber-900">{t('cart.coupon_terms_title')}</h3>{couponPreview.coupon.description && <p className="mt-1 text-xs text-amber-800">{couponPreview.coupon.description}</p>}<ul className="mt-2 space-y-1 text-xs text-amber-900"><li>{couponBenefit(couponPreview, t)}</li>{couponPreview.coupon.minOrderAmount !== null && <li>{t('cart.coupon_minimum', { amount: formatRWF(couponPreview.coupon.minOrderAmount) })}</li>}{couponPreview.coupon.maxDiscountAmount !== null && <li>{t('cart.coupon_maximum', { amount: formatRWF(couponPreview.coupon.maxDiscountAmount) })}</li>}{couponPreview.coupon.endsAt && <li>{t('cart.coupon_expires', { date: new Date(couponPreview.coupon.endsAt).toLocaleDateString() })}</li>}{couponPreview.coupon.usageRemaining !== null && <li>{t('cart.coupon_uses_remaining', { count: couponPreview.coupon.usageRemaining })}</li>}<li>{t('cart.coupon_per_customer', { count: couponPreview.coupon.usageLimitPerUser })}</li>{!couponPreview.coupon.appliesToAllProducts && <li>{t('cart.coupon_selected_only')}</li>}</ul>{couponPreview.canApply && <p className="mt-2 flex items-center gap-1 text-sm font-bold text-emerald-700"><Check className="h-4 w-4" />{couponPreview.freeShipping ? t('cart.coupon_free_delivery') : t('cart.coupon_will_save', { amount: formatRWF(couponPreview.discountAmount) })}</p>}<button type="button" onClick={applyCoupon} disabled={!couponPreview.canApply} className="mt-3 min-h-11 w-full rounded-[10px] bg-fcs-brand-strong font-bold text-white disabled:cursor-not-allowed disabled:opacity-40">{t('cart.apply_coupon')}</button></div>}</div> : <div className="mt-4 flex items-center justify-between rounded-xl bg-emerald-50 p-3"><span className="text-sm font-bold text-emerald-800">{appliedCoupon.code}</span><IconButton label={t('cart.remove_coupon')} icon={<X className="h-4 w-4" />} onClick={() => { clearCoupon(); setCouponCode('') }} variant="ghost" /></div>)}
      {isWholesale ? <><p className="mt-4 flex items-center gap-2 rounded-xl bg-emerald-50 p-3 text-sm font-bold text-emerald-800"><Check className="h-4 w-4" />Wholesale prices applied</p><WholesaleCartOrderButton items={cart.items} managerWhatsApp={user?.assignedManagerWhatsApp} onClearCart={cart.clearCart} className="mt-3" /></> : <><Link href="/checkout" className="mt-5 hidden min-h-12 items-center justify-center gap-2 rounded-[10px] bg-fcs-brand-strong font-bold text-white md:flex">{t('cart.checkout')}<ArrowRight className="h-4 w-4" /></Link><CartWhatsAppOrder items={cart.items} subtotal={cart.subtotal} discount={discount} district={cart.selectedDistrict} deliveryFee={delivery ? deliveryFee : null} /></>}</aside></div>
    {cart.items.length > 0 && <div className="h-24 md:hidden" />}<div className="fixed bottom-[calc(64px+env(safe-area-inset-bottom))] left-0 right-0 z-40 border-t border-[#EEEEEE] bg-white p-3 shadow-[0_-2px_12px_rgba(0,0,0,0.06)] md:hidden"><div className="flex items-center gap-3"><div className="min-w-0 flex-1"><p className="text-xs text-gray-500">{t('cart.items', { count: cart.itemCount })}</p><p className="text-lg font-bold text-fcs-brand-text">{formatRWF(total)}</p></div>{isWholesale ? <WholesaleCartOrderButton items={cart.items} managerWhatsApp={user?.assignedManagerWhatsApp} onClearCart={cart.clearCart} className="max-w-[230px] px-3 text-sm" /> : <Link href="/checkout" className="flex min-h-[52px] items-center rounded-[10px] bg-fcs-brand-strong px-6 font-bold text-white">{t('cart.checkout')}</Link>}</div></div>
  </main>
}
...
function CartRow({ item, isWholesale, onRemove, onSave, onQuantity }: { item: CartItem; isWholesale: boolean; onRemove: () => void; onSave: () => void; onQuantity: (quantity: number) => void }) {
  const { t } = useLanguage()
  const image = item.imagePublicId ? `https://res.cloudinary.com/dohoc0tmp/image/upload/w_300,h_300,c_fill,q_auto,f_auto/${item.imagePublicId}` : item.image
  const retailPrice = item.retailPrice || item.price
  const savings = Math.max(0, retailPrice - item.price)
  return <article className="rounded-xl border border-[#EEEEEE] bg-white p-4">
    <div className="flex gap-3"><Link href={item.isBundle ? `/bundles/${item.slug}` : `/products/${item.slug}`} className="relative h-20 w-20 shrink-0 overflow-hidden rounded-xl bg-gray-50">{image && <Image src={image} alt={item.imageAlt || item.name} fill sizes="80px" className="object-contain p-1" />}</Link><div className="min-w-0 flex-1"><h2 className="line-clamp-2 text-sm font-bold">{item.name}</h2>{item.volume && <p className="text-xs text-fcs-text-muted">{item.volume}</p>}<p className="mt-1 text-xs text-gray-600">{formatRWF(item.price)} each</p><p className="font-bold text-fcs-brand-text">{formatRWF(item.price * item.quantity)}</p>{isWholesale && savings > 0 && <><p className="mt-1 text-xs text-fcs-text-muted line-through">Retail: {formatRWF(retailPrice)}</p><p className="text-xs font-semibold text-emerald-700">Save: {formatRWF(savings)} per unit</p></>}<p className="mt-1 text-xs text-gray-500">{t('cart.stock_available', { count: item.maxQuantity })}</p></div><IconButton label={`${t('cart.remove')}: ${item.name}`} icon={<Trash2 className="h-4 w-4" />} variant="danger" onClick={onRemove} /></div>
    <div className="mt-3 flex items-center justify-between border-t pt-3"><div className="flex h-11 items-center overflow-hidden rounded-xl border"><IconButton label={`${t('product.decrease_quantity')}: ${item.name}`} icon={<Minus className="h-4 w-4" />} onClick={() => onQuantity(item.quantity - 1)} disabled={item.quantity <= 1} className="rounded-none" /><span className="grid h-11 min-w-11 place-items-center border-x text-sm font-bold">{item.quantity}</span><IconButton label={`${t('product.increase_quantity')}: ${item.name}`} icon={<Plus className="h-4 w-4" />} onClick={() => onQuantity(item.quantity + 1)} disabled={item.quantity >= item.maxQuantity} className="rounded-none" /></div><button type="button" onClick={onSave} className="flex min-h-11 items-center gap-1.5 px-2 text-xs font-bold text-gray-500"><Heart className="h-4 w-4" />{t('cart.save_for_later')}</button></div>{item.quantity >= item.maxQuantity && <p className="mt-2 text-xs font-semibold text-amber-700">{t('cart.max_reached')}</p>}
  </article>
}
function Summary({ label, value, green, strong }: { label: string; value: string; green?: boolean; strong?: boolean }) { return <p className={`mt-3 flex justify-between text-sm ${strong ? 'text-base font-bold' : ''} ${green ? 'text-emerald-700' : ''}`}><span>{label}</span><span>{value}</span></p> }
