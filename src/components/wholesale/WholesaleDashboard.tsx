  ...
  return (
    <main className="mx-auto max-w-6xl px-4 py-6 sm:px-6">
      ...
      <section className="mt-6 rounded-2xl border bg-white p-5">
        ...
        <div className="mt-3 flex flex-wrap items-center gap-4"><span className="grid h-14 w-14 place-items-center rounded-full bg-rose-50 text-lg font-bold text-fcs-brand-text">{managerLabel.split(/\s+/).map((part) => part[0]).join('').slice(0, 2).toUpperCase()}</span><div className="min-w-0 flex-1"><p className="font-bold">{managerLabel}</p><p className="text-sm text-gray-500">{manager ? 'Wholesale Account Manager' : 'Wholesale support contact'}</p>{!manager && <p className="mt-1 text-xs text-amber-700">A dedicated manager has not been assigned yet.</p>}</div><div className="flex gap-2">{whatsapp && <a href={`https://wa.me/${whatsapp}?text=${message}`} target="_blank" rel="noreferrer"><Button><MessageCircle className="mr-2 h-4 w-4" />WhatsApp</Button></a>}{manager?.phone && <a href={`tel:${manager.phone}`}><Button variant="outline"><Phone className="mr-2 h-4 w-4" />Call</Button></a>}</div></div>
      </section>
      ...
      {data.frequentProducts.length > 0 && <section className="mt-6"><h2 className="font-bold">Frequently ordered</h2><div className="mt-3 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{data.frequentProducts.map((item) => <article key={item.id} className="rounded-xl border bg-white p-4"><div className="flex gap-3">{item.image ? <img src={thumbnailImageUrl(item.image, 64)} alt="" loading="lazy" decoding="async" className="h-16 w-16 rounded-lg object-contain" /> : <span className="grid h-16 w-16 place-items-center rounded-lg bg-gray-100"><Package className="h-6 w-6 text-gray-300" /></span>}<div><p className="font-semibold">{item.name}</p><p className="text-xs text-gray-500">Ordered {item.orderCount} time{item.orderCount === 1 ? '' : 's'}</p><p className="mt-1 text-sm font-bold">Retail reference: {formatRWF(item.retailPrice)}</p></div></div><Button className="mt-3" size="sm" variant="outline" onClick={openWholesaleCatalog}>View your current price</Button></article>)}</div></section>}

      <section className="mt-6 grid gap-6 lg:grid-cols-2"><div><h2 className="font-bold">Recent orders</h2><div className="mt-3 space-y-2">{data.recentOrders.length === 0 ? <Empty text="No wholesale orders yet." /> : data.recentOrders.map((order) => <div key={order.id} className="flex items-center justify-between rounded-xl border bg-white p-3"><div><p className="font-mono text-xs font-bold">{order.orderNumber}</p><OrderStatusBadge status={order.status} className="mt-1" /></div><div className="text-right"><p className="font-bold">{formatRWF(order.total)}</p><button className="text-xs text-fcs-brand-text" onClick={() => reorder(order.id)}>Reorder</button></div></div>)}</div></div><div><h2 className="font-bold">Recent invoices</h2><div className="mt-3 space-y-2">{data.recentInvoices.length === 0 ? <Empty text="Invoices are generated from wholesale orders." /> : data.recentInvoices.map((invoice) => <button key={invoice.id} onClick={onInvoices} className="flex w-full justify-between rounded-xl border bg-white p-3 text-left"><span><span className="block font-mono text-xs font-bold">{invoice.invoiceNumber}</span><span className="text-xs text-gray-500">{new Date(invoice.issuedAt).toLocaleDateString('en-RW')}</span></span><span className="font-bold">{formatRWF(invoice.totalAmount)}</span></button>)}</div></div></section>
...
function Stat({ label, value, icon }: { label: string; value: string; icon: ReactNode }) { return <div className="rounded-2xl border bg-white p-4"><span className="text-fcs-brand-text [&_svg]:h-5 [&_svg]:w-5">{icon}</span><p className="mt-2 break-words text-lg font-bold">{value}</p><p className="text-xs uppercase tracking-wider text-gray-500">{label}</p></div> }
