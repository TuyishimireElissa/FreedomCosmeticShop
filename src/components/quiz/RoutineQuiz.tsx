      ...
      <div className="mx-auto max-w-2xl">
        {phase === 'questions' && <section aria-labelledby="quiz-question">
          <div className="mb-6 flex items-center gap-3">
            ...
            <div className="flex-1"><p className="mb-2 text-xs font-bold text-gray-500">{t('quiz.step', { step: stepNumber, total: 6 })}</p><div className="h-2 overflow-hidden rounded-full bg-gray-200"><div className="h-full rounded-full bg-fcs-brand transition-[width] duration-300" style={{ width: `${progress}%` }} /></div></div>
          </div>
          <div className="mb-6"><span className="mb-4 grid h-12 w-12 place-items-center rounded-2xl bg-fcs-brand-strong text-white"><Sparkles className="h-5 w-5" /></span><h1 id="quiz-question" className="text-2xl font-black text-gray-900 sm:text-3xl">{t(step.questionKey)}</h1>{step.subtitleKey && <p className="mt-2 text-sm leading-6 text-gray-500">{t(step.subtitleKey)}</p>}</div>
          <div className="space-y-3">{step.options.map((option) => <button key={option.value} type="button" onClick={() => choose(option.value)} className="flex min-h-[72px] w-full items-center gap-4 rounded-2xl border-2 border-gray-100 bg-white p-4 text-left shadow-sm transition-all hover:border-fcs-brand active:scale-[0.98]"><span className="text-3xl" aria-hidden="true">{option.icon}</span><span className="flex-1 text-base font-bold text-gray-900">{t(option.labelKey)}</span><ArrowRight className="h-5 w-5 text-gray-300" /></button>)}</div>
        </section>}

        {phase === 'loading' && <section className="flex min-h-[60vh] flex-col items-center justify-center text-center"><span className="grid h-20 w-20 place-items-center rounded-full bg-rose-100"><Sparkles className="h-9 w-9 animate-pulse text-fcs-brand-text" /></span><h1 className="mt-5 text-xl font-black text-gray-900">{t('quiz.loading')}</h1><p className="mt-2 text-sm text-gray-500">{t('quiz.loading_subtitle')}</p><Loader2 className="mt-6 h-6 w-6 animate-spin text-fcs-brand-text" /></section>}

        {phase === 'results' && <section>
          <div className="mb-7 text-center"><span className="mx-auto grid h-16 w-16 place-items-center rounded-full bg-rose-100"><Sparkles className="h-7 w-7 text-fcs-brand-text" /></span><h1 className="mt-4 text-2xl font-black text-gray-900">{t('quiz.results_title')}</h1><p className="mt-2 text-sm text-gray-500">{t('quiz.results_subtitle')}</p></div>
          {error ? <div className="rounded-2xl bg-red-50 p-6 text-center"><p className="text-sm font-semibold text-red-700">{t('errors.load_failed')}</p><button type="button" onClick={restart} className="mt-4 min-h-12 rounded-xl bg-gray-900 px-5 font-bold text-white">{t('quiz.retry')}</button></div> : <>
            ...
            {recommendations?.bundles && recommendations.bundles.length > 0 && <section className="mb-8"><h2 className="mb-4 text-lg font-black text-gray-900">{t('quiz.recommended_bundles')}</h2><div className="scrollbar-hide flex snap-x gap-3 overflow-x-auto pb-2">{recommendations.bundles.map((bundle) => { const name = language === 'rw' && bundle.nameRw ? bundle.nameRw : bundle.name; return <article key={bundle.id} className="w-[82vw] max-w-sm flex-none snap-start rounded-2xl border border-rose-100 bg-white p-4"><h3 className="font-bold text-gray-900">{name}</h3><p className="mt-2 text-sm text-gray-500">{bundle.products.length} · {formatRWF(bundle.normalTotal)}</p><p className="mt-2 text-lg font-black text-fcs-brand-text">{formatRWF(bundle.bundlePrice)}</p>{bundle.savings > 0 && <p className="text-sm font-bold text-emerald-700">{formatRWF(bundle.savings)}</p>}</article>})}</div></section>}
          ...
          <div className="mt-7 flex flex-col gap-3"><Link href="/products" className="flex min-h-12 items-center justify-center rounded-xl bg-fcs-brand-strong px-5 font-bold text-white">{t('quiz.view_all')}</Link><button type="button" onClick={restart} className="min-h-12 rounded-xl border-2 border-gray-200 font-bold text-gray-700">{t('quiz.restart')}</button></div>
        </section>}
      </div>
    </main>
  )
