    ...
    <main className="min-h-[calc(100dvh-4rem)] bg-gradient-to-b from-rose-50 to-white px-4 py-6">
      <div className="mx-auto max-w-2xl">
        {phase === 'questions' && <section aria-labelledby="quiz-question">
          ...
          <div className="mb-6"><span className="mb-4 grid h-12 w-12 place-items-center rounded-2xl bg-fcs-brand text-white"><Sparkles className="h-5 w-5" /></span><h1 id="quiz-question" className="text-2xl font-black text-gray-900 sm:text-3xl">{t(step.questionKey)}</h1>{step.subtitleKey && <p className="mt-2 text-sm leading-6 text-gray-500">{t(step.subtitleKey)}</p>}</div>
        ...
        {phase === 'results' && <section>
          ...
          <div className="mt-7 flex flex-col gap-3"><Link href="/products" className="flex min-h-12 items-center justify-center rounded-xl bg-fcs-brand px-5 font-bold text-white">{t('quiz.view_all')}</Link><button type="button" onClick={restart} className="min-h-12 rounded-xl border-2 border-gray-200 font-bold text-gray-700">{t('quiz.restart')}</button></div>
        </section>}
      </div>
    </main>
  )
