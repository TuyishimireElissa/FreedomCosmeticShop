...
export default function WhatsAppSupportView() {
  ...
  if (!WA_CONFIG.isNumberConfigured) return <main className="mx-auto max-w-xl px-4 py-12 text-center"><MessageCircle className="mx-auto h-12 w-12 text-gray-300" /><h1 className="mt-4 text-2xl font-black">WhatsApp support is not configured</h1><p className="mt-2 text-sm text-gray-500">Use the contact page for currently available support channels.</p><Link href="/contact" className="mt-6 inline-flex min-h-12 items-center rounded-xl bg-gray-900 px-5 font-bold text-white">{t('confirmation.contact_support')}</Link></main>
  ...
  return <main className="mx-auto max-w-2xl px-4 py-8 sm:py-12">
    ...
    <section className="mt-7 rounded-2xl border bg-white p-5 shadow-sm"><div className="flex items-center gap-3"><span className="grid h-14 w-14 place-items-center rounded-full bg-rose-50 text-fcs-brand-text"><User className="h-7 w-7" /></span><div><p className="font-black text-gray-900">{WA_CONFIG.agentName || t('whatsapp.agent_name_unpublished')}</p><p className="text-sm text-gray-500">{t('whatsapp.assisted_support')}</p><p className="mt-1 text-xs text-amber-700">{t('whatsapp.availability_not_claimed')}</p></div></div>
      <div className="mt-4 rounded-xl bg-gray-50 p-4"><h2 className="flex items-center gap-2 text-sm font-black text-gray-900"><Clock className="h-4 w-4 text-fcs-brand-text" />{t('whatsapp.support_hours')}</h2>{WA_CONFIG.responseHours.weekdays ? <div className="mt-2 space-y-1 text-sm text-gray-600"><p>{WA_CONFIG.responseHours.weekdays}</p>{WA_CONFIG.responseHours.saturday && <p>{WA_CONFIG.responseHours.saturday}</p>}{WA_CONFIG.responseHours.sunday && <p>{WA_CONFIG.responseHours.sunday}</p>}<p className="text-xs text-fcs-text-muted">{WA_CONFIG.responseHours.timezone}</p>{WA_CONFIG.responseHours.responseTime && <p className="border-t pt-2 text-xs">{t('whatsapp.response_time_value', { time: WA_CONFIG.responseHours.responseTime })}</p>}</div> : <p className="mt-2 text-sm leading-6 text-amber-700">{t('whatsapp.hours_unpublished')}</p>}</div>
    </section>
