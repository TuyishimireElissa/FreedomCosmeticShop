          ...
          <article key={product.id} className="group flex min-w-0 flex-col overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-[0_5px_20px_rgba(26,26,26,0.05)] transition-all hover:-translate-y-1 hover:border-rose-100 hover:shadow-[0_16px_34px_rgba(183,110,121,0.14)]">
            <div className="relative aspect-square overflow-hidden bg-[#f8f9fa]">
              <Link href={`/products/${product.slug}`} className="block h-full">
                {image ? <img src={image} alt={product.name} className={`h-full w-full object-cover transition-transform duration-500 group-hover:scale-105 ${outOfStock ? 'opacity-60' : ''}`} loading="lazy" /> : <div className="grid h-full place-items-center text-xs text-gray-400">{t('product.no_image')}</div>}
              </Link>
            ...
            <div className="flex flex-1 flex-col p-3 sm:p-4">
              ...
              <button type="button" disabled={outOfStock} onClick={() => { addToCart({ productId: product.id, slug: product.slug, name: product.name, price: product.price, image: image || '', stock: product.stock }); toast({ title: t('product.added'), description: product.name }) }} className="mt-3 flex min-h-11 w-full items-center justify-center gap-2 rounded-xl bg-[#B76E79] px-3 text-xs font-bold text-white transition-colors hover:bg-[#a55d68] disabled:cursor-not-allowed disabled:bg-gray-300 sm:text-sm"><ShoppingBag className="h-4 w-4" />{outOfStock ? t('common.sold_out') : t('product.add_to_cart')}</button>
            </div>
          </article>
        )
      })}
