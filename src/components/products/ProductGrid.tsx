  ...
  return (
    <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 sm:gap-4 lg:grid-cols-4">
      {products.map((product) => {
        ...
        return (
          <article key={product.id} className="group flex min-w-0 flex-col overflow-hidden rounded-xl border border-[#EEEEEE] bg-white shadow-[0_1px_3px_rgba(0,0,0,0.04)] transition-all duration-200 hover:-translate-y-0.5 hover:shadow-[0_4px_12px_rgba(0,0,0,0.08)]">
            <div className="relative aspect-square overflow-hidden bg-[#FAFAFA]">
              <Link href={`/products/${product.slug}`} className="block h-full">
                {image ? <SmartImage publicId={primaryImage?.publicId || undefined} fallbackSrc={primaryImage?.url || undefined} context="card" alt={imageAlt} fill aspectRatio={1} className={`object-contain p-3 transition-transform duration-200 group-hover:scale-[1.02] ${outOfStock ? 'opacity-60' : ''}`} /> : <div className="grid h-full place-items-center text-xs text-gray-400">{t('product.no_image')}</div>}
              </Link>
              <div className="absolute left-2 top-2">{outOfStock ? <span className="rounded-full bg-[#1a1a1a] px-2.5 py-1 text-[10px] font-medium text-white">{t('common.sold_out')}</span> : discount > 0 ? <span className="rounded-full bg-[#D64045] px-2.5 py-1 text-[10px] font-medium text-white">-{discount}%</span> : product.isBestSeller === true ? <span className="rounded-full bg-[#B76E79] px-2.5 py-1 text-[10px] font-medium text-white">{t('categories.best_sellers')}</span> : product.isNewArrival === true ? <span className="rounded-full bg-[#2D8A4E] px-2.5 py-1 text-[10px] font-medium text-white">{t('common.new')}</span> : lowStock ? <span className="rounded-full bg-[#E8A838] px-2.5 py-1 text-[10px] font-medium text-white">{t('common.low_stock', { count: product.stock })}</span> : null}</div>
            ...
            <div className="flex flex-1 flex-col p-3 sm:p-4">
              <p className="truncate text-[10px] font-bold uppercase tracking-[0.14em] text-gray-400">{product.brand?.name || product.category?.name || ''}</p>
              ...
              {(product.volume || product.size) && <p className="mt-1 text-[11px] text-[#777777]">{product.volume || product.size}</p>}
              ...
              <button type="button" disabled={outOfStock} onClick={() => { addToCart({ productId: product.id, slug: product.slug, name: product.name, price: product.price, image: image || '', stock: product.stock }); toast({ title: t('product.added'), description: product.name }) }} className="mt-3 flex min-h-12 w-full items-center justify-center gap-2 rounded-[10px] bg-[#B76E79] px-3 text-[13px] font-semibold text-white transition-all duration-150 hover:bg-[#9B5A64] active:scale-[0.98] disabled:cursor-not-allowed disabled:bg-gray-300 sm:text-sm"><ShoppingBag className="h-4 w-4" />{outOfStock ? t('common.sold_out') : t('product.add_to_cart')}</button>
            </div>
          </article>
        )
      })}
