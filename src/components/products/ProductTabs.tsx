      ...
      <div className="scrollbar-hide flex overflow-x-auto border-b border-gray-100 px-2 sm:px-5" role="tablist" aria-label={t('product.product_information')}>
        {tabs.map((tab) => (
          <button
            ...
            className={`shrink-0 border-b-2 px-4 py-4 text-sm font-bold transition-colors ${active === tab.id ? 'border-fcs-brand text-fcs-brand-text' : 'border-transparent text-gray-500 hover:text-gray-800'}`}
          >
            {tab.label}
          </button>
        ))}
      ...
      <div id={`product-panel-${active}`} role="tabpanel" aria-labelledby={`product-tab-${active}`} className="p-5 sm:p-7">
        {active === 'description' && (
          <div className="text-gray-600">
            ...
            {fragranceGroups.length > 0 && <div className="mt-6"><h3 className="flex items-center gap-2 font-bold text-gray-800"><Leaf className="h-4 w-4 text-fcs-brand-text" />{t('product.fragrance_notes')}</h3><div className="mt-3 grid gap-3 sm:grid-cols-3">{fragranceGroups.map(([label, notes]) => <Detail key={label} label={label} value={notes.join(', ')} />)}</div></div>}
          </div>
        )}

        {active === 'ingredients' && (
          <div>
            <h2 className="flex items-center gap-2 text-lg font-bold text-[#1a1a1a]"><FlaskConical className="h-5 w-5 text-fcs-brand-text" />{t('product.ingredients')}</h2>
            {ingredients.length > 0 && <div className="mt-4 flex flex-wrap gap-2">{ingredients.map((ingredient, index) => <span key={`${ingredient}-${index}`} className="rounded-full border border-rose-100 bg-rose-50 px-3 py-1.5 text-sm font-semibold text-fcs-brand-text">{ingredient}</span>)}</div>}
        ...
        {active === 'results' && (
          <div>
            <h2 className="flex items-center gap-2 text-lg font-bold text-[#1a1a1a]"><Sparkles className="h-5 w-5 text-fcs-brand-text" />{t('product.expected_results')}</h2>
        ...
        {active === 'delivery' && <div><h2 className="flex items-center gap-2 text-lg font-bold text-[#1a1a1a]"><Truck className="h-5 w-5 text-fcs-brand-text" />{t('product.delivery_across_rwanda')}</h2><p className="mt-2 text-sm leading-6 text-gray-600">{t('product.delivery_exact_fee')}</p><div className="mt-5"><DeliveryEstimator orderTotal={product.price} /></div><p className="mt-4 text-xs leading-5 text-gray-500">{t('product.return_policy_short')}</p></div>}
      </div>
    </section>
  )
}
...
function Detail({ label, value, icon }: { label: string; value: string; icon?: React.ReactNode }) {
  return <div className="rounded-xl bg-[#f8f9fa] p-4"><p className="flex items-center gap-1.5 text-xs font-bold uppercase tracking-wider text-fcs-text-muted">{icon}{label}</p><p className="mt-1.5 text-sm font-semibold leading-6 text-gray-800">{value}</p></div>
}
