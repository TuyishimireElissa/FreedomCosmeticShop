...
import { buildProductOrderMessage, buildWhatsAppUrl, trackWhatsAppClick, WA_CONFIG } from '@/lib/whatsapp-service'
...
export default function OrderViaWhatsApp({ product, selectedShade, selectedSize, quantity, className = '', variant = 'full' }: OrderViaWhatsAppProps) {
  ...
  const disabled = !WA_CONFIG.isNumberConfigured || product.stock < 1 || quantity < 1 || quantity > product.stock
