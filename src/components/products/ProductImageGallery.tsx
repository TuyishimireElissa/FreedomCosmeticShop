...
import { getProductImageGallery, optimizedImageSrcSet, optimizedImageUrl, type StructuredProductImage } from '@/lib/cloudinary-images'
...
export default function ProductImageGallery({
  ...
  return (
    <div className="space-y-3">
      ...
      >
        {failedUrl !== activeImage.url ? <img
          src={optimizedImageUrl(activeImage.url, 800)}
          srcSet={optimizedImageSrcSet(activeImage.url, [600, 800])}
      ...
      {gallery.length > 1 && <p className="text-center text-xs text-gray-500">{activeIndex + 1} / {gallery.length}</p>}
      {isAuthentic && <p className="text-center text-xs text-gray-500">{t('product.authenticity_verified')}</p>}
    </div>
  )
}
