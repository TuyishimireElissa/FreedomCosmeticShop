  ...
  return (
    <div className="space-y-3">
      <div className="group relative aspect-square overflow-hidden rounded-3xl border border-gray-100 bg-[#f8f9fa] shadow-sm">
        {current ? <Image src={current} alt={t('product.image_number', { product: productName, number: active + 1 })} fill priority={active === 0} sizes="(max-width: 768px) 100vw, 50vw" className={`object-cover transition-transform duration-300 md:group-hover:scale-110 ${outOfStock ? 'opacity-60' : ''}`} /> : <div className="grid h-full place-items-center text-sm text-gray-400">{t('product.no_image_available')}</div>}
      ...
      {safeImages.length > 1 && <div className="scrollbar-hide flex gap-2 overflow-x-auto pb-1">{safeImages.map((image, index) => <button key={`${image}-${index}`} type="button" onClick={() => setActive(index)} className={`h-16 w-16 shrink-0 overflow-hidden rounded-xl border-2 transition-all sm:h-20 sm:w-20 ${active === index ? 'border-[#B76E79] shadow-md' : 'border-transparent opacity-70 hover:opacity-100'}`} aria-label={t('product.show_image', { number: index + 1 })}><Image src={image} alt="" width={80} height={80} sizes="80px" className="h-full w-full object-cover" loading="lazy" /></button>)}</div>}
    </div>
  )
}
