  ...
  return (
    <aside className={cn('sticky top-24 hidden w-60 shrink-0 self-start rounded-xl border border-[#EEEEEE] bg-white p-4 lg:block', className)} aria-label={t('search.filters')}>
      <div className="mb-4 flex min-h-9 items-center justify-between border-b border-[#EEEEEE] pb-3">
        ...
        {activeFilterCount > 0 && <button type="button" onClick={clearAllFilters} className="min-h-9 rounded-lg px-2 text-xs font-semibold text-fcs-brand-text hover:bg-rose-50">{t('search.clear_all_filters')}</button>}
      </div>
      ...
      <div className="max-h-[calc(100vh-9rem)] overflow-y-auto pr-1">
        ...
        <FilterSection title={t('search.price_range_label')}>
          ...
          <div className="mt-3 grid grid-cols-2 gap-2">
            <label className="text-[10px] font-semibold uppercase tracking-wide text-fcs-text-muted">{t('search.minimum')}<input type="number" min="0" step="500" value={filters.minPrice} onChange={(event) => setFilter('minPrice', event.target.value)} className="mt-1 min-h-10 w-full rounded-lg border border-gray-200 bg-white px-2 text-sm text-gray-800 outline-none transition-colors focus:border-fcs-brand" /></label>
            <label className="text-[10px] font-semibold uppercase tracking-wide text-fcs-text-muted">{t('search.maximum')}<input type="number" min="0" step="500" value={filters.maxPrice} onChange={(event) => setFilter('maxPrice', event.target.value)} className="mt-1 min-h-10 w-full rounded-lg border border-gray-200 bg-white px-2 text-sm text-gray-800 outline-none transition-colors focus:border-fcs-brand" /></label>
          </div>
        </FilterSection>
