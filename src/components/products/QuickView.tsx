  ...
  >
    ...
    >
      ...
      <section className="relative h-[38vh] min-h-64 shrink-0 overflow-hidden bg-gradient-to-b from-gray-50 to-white md:h-auto md:min-h-[620px] md:w-[45%]">
        {activeImage ? <button type="button" onClick={() => setZoomed((value) => !value)} className="relative h-full w-full overflow-hidden" aria-label={zoomed ? 'Zoom out product image' : 'Zoom in product image'}>
      ...
      <section className="ub-scroll flex-1 overflow-y-auto px-5 pb-[calc(24px+env(safe-area-inset-bottom))] pt-5 md:w-[55%] md:px-8 md:py-8">
        <p className="text-[11px] font-bold uppercase tracking-[0.18em] text-gray-400">{product.brand?.name || product.category?.name || t('product.beauty')}</p>
        ...
        <div className="flex flex-wrap items-baseline gap-3"><span className="text-3xl font-black tracking-tight text-[#B76E79]">{formatRWF(displayPrice)}</span>{comparePrice && <span className="text-sm font-semibold text-gray-400 line-through">{formatRWF(comparePrice)}</span>}{discount > 0 && <span className="rounded-lg bg-red-500 px-2.5 py-1 text-xs font-bold text-white">-{discount}%</span>}</div>
        {savings > 0 && <p className="mt-1 text-sm font-bold text-emerald-700">You save {formatRWF(savings)}{isWholesale ? ' per unit' : ''}</p>}
        {isWholesale && <span className="mt-3 inline-flex rounded-full bg-emerald-100 px-3 py-1 text-xs font-black uppercase tracking-wider text-emerald-800">Wholesale</span>}
        ...
        <div className="mt-5 flex items-center justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-wider text-gray-500">Quantity</p><div className="mt-1 flex h-12 items-center overflow-hidden rounded-xl border border-gray-200"><IconButton label={t('product.decrease_quantity')} icon={<Minus className="h-4 w-4" />} onClick={() => setQuantity((value) => Math.max(1, value - 1))} disabled={quantity <= 1} className="h-full rounded-none" /><span className="grid h-full min-w-12 place-items-center border-x text-sm font-black" aria-live="polite">{quantity}</span><IconButton label={t('product.increase_quantity')} icon={<Plus className="h-4 w-4" />} onClick={() => setQuantity((value) => Math.min(maxQuantity, value + 1))} disabled={quantity >= maxQuantity} className="h-full rounded-none" /></div></div><div className="text-right"><p className="text-xs font-bold uppercase tracking-wider text-gray-500">Total</p><p className="mt-1 text-xl font-black text-gray-950" aria-live="polite">{formatRWF(total)}</p></div></div>

        <button type="button" onClick={addProduct} disabled={outOfStock} className={`mt-5 flex min-h-14 w-full items-center justify-center gap-2 rounded-xl px-4 text-base font-black text-white transition-all active:scale-[0.98] disabled:cursor-not-allowed disabled:bg-gray-300 ${added || isWholesale ? 'bg-emerald-600 hover:bg-emerald-700' : 'bg-[#B76E79] hover:bg-[#9B5A64]'}`}>{added ? <><Check className="h-5 w-5" />Added!</> : <><ShoppingCart className="h-5 w-5" />{outOfStock ? t('common.sold_out') : `${t('product.add_to_cart')} — ${formatRWF(total)}`}</>}</button>

        <Link href={`/products/${product.slug}`} onClick={() => { historyPushed.current = false; window.history.replaceState({ ...window.history.state, fcsQuickView: false }, '') }} className="mt-3 flex min-h-12 items-center justify-center gap-2 rounded-xl border-2 border-gray-200 text-sm font-bold text-gray-800 hover:border-[#B76E79] hover:text-[#B76E79]">View Full Details<ExternalLink className="h-4 w-4" /></Link>
        <div className="mt-3 grid grid-cols-2 gap-3"><button type="button" onClick={() => void toggleWishlist()} disabled={wishlistBusy} className="flex min-h-12 items-center justify-center gap-2 rounded-xl bg-gray-50 text-sm font-bold text-gray-700 hover:bg-rose-50 hover:text-[#B76E79]"><Heart className={`h-4 w-4 ${wishlisted ? 'fill-red-500 text-red-500' : ''}`} />{wishlisted ? 'Saved' : 'Wishlist'}</button><button type="button" onClick={shareProduct} className="flex min-h-12 items-center justify-center gap-2 rounded-xl bg-emerald-50 text-sm font-bold text-emerald-800 hover:bg-emerald-100"><Share2 className="h-4 w-4" />WhatsApp</button></div>

        <div className="mt-5 grid gap-2 rounded-xl bg-gray-50 p-4 text-sm"><p className={`font-bold ${outOfStock ? 'text-red-600' : product.stock <= 5 ? 'text-amber-700' : 'text-emerald-700'}`}>{outOfStock ? t('common.sold_out') : product.stock <= 5 ? `${product.stock} left in stock` : 'In stock'}</p>{delivery?.deliveryTime && <p className="text-gray-600"><span className="font-bold text-gray-800">Kigali:</span> {delivery.deliveryTime}{delivery.feeFormatted ? ` · ${delivery.feeFormatted}` : ''}</p>}</div>
      </section>
    </div>
  </div>
}
