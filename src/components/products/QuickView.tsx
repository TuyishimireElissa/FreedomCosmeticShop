  ...
  >
    ...
    >
      ...
      <section className="ub-scroll flex-1 overflow-y-auto px-5 pb-[calc(24px+env(safe-area-inset-bottom))] pt-5 md:w-[55%] md:px-8 md:py-8">
        ...
        <button type="button" onClick={addProduct} disabled={outOfStock} className={`mt-5 flex min-h-14 w-full items-center justify-center gap-2 rounded-xl px-4 text-base font-black text-white transition-all active:scale-[0.98] disabled:cursor-not-allowed disabled:bg-gray-300 ${added || isWholesale ? 'bg-emerald-600 hover:bg-emerald-700' : 'bg-fcs-brand hover:bg-[#9B5A64]'}`}>{added ? <><Check className="h-5 w-5" />Added!</> : <><ShoppingCart className="h-5 w-5" />{outOfStock ? t('common.sold_out') : `${t('product.add_to_cart')} — ${formatRWF(total)}`}</>}</button>
