      ...
      <div className="mx-auto max-w-7xl px-4 py-5 sm:px-6 sm:py-8 lg:px-8">
        ...
        <div className="grid gap-8 lg:grid-cols-2 lg:gap-12">
          ...
          <div className="min-w-0">
            ...
            <div className="mt-5 flex flex-wrap items-baseline gap-3"><span className="text-3xl font-bold text-[#B76E79]">{formatRWF(product.price)}</span>{product.compareAt && product.compareAt > product.price && <span className="text-base text-gray-400 line-through">{formatRWF(product.compareAt)}</span>}{discount > 0 && <span className="rounded-full bg-red-50 px-2.5 py-1 text-xs font-bold text-red-600">{t('product.save_percent', { percent: discount })}</span>}</div>
