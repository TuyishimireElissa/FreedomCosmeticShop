      ...
      <div className="mx-auto max-w-7xl px-4 py-5 sm:px-6 sm:py-8 lg:px-8">
        ...
        <div className="grid gap-8 lg:grid-cols-2 lg:gap-12">
          ...
          <div className="min-w-0">
            ...
            {product.reviewsCount > 0 && <button type="button" onClick={() => document.getElementById('product-details')?.scrollIntoView({ behavior: 'smooth' })} className="mt-3 flex items-center gap-2"><span className="flex">{[1,2,3,4,5].map((star) => <Star key={star} className={`h-4 w-4 ${star <= Math.round(product.rating) ? 'fill-[#FFD700] text-[#FFD700]' : 'fill-gray-200 text-gray-200'}`} />)}</span><span className="text-sm font-bold">{product.rating.toFixed(1)}</span><span className="text-xs text-gray-400">{t('product.reviews_count', { count: product.reviewsCount })}</span></button>}
