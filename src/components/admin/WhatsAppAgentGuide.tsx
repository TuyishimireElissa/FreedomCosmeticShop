...
export default function WhatsAppAgentGuide() {
  ...
  return <main className="mx-auto max-w-5xl"><header><div className="flex items-center gap-2"><MessageCircle className="h-6 w-6 text-green-600" /><h1 className="text-2xl font-black text-gray-900">{t('whatsapp.guide_title')}</h1></div><p className="mt-2 text-sm text-gray-500">{t('whatsapp.guide_subtitle')}</p></header>
    ...
    <div className="mt-6 grid gap-5 md:grid-cols-[220px_1fr]"><nav className="flex gap-2 overflow-x-auto pb-2 md:block md:space-y-2" aria-label={t('whatsapp.guide_topics')}>{sections.map((item) => <button type="button" key={item.id} onClick={() => setSectionId(item.id)} className={`min-h-11 shrink-0 rounded-xl px-4 text-left text-sm font-bold md:w-full ${sectionId === item.id ? 'bg-fcs-brand-strong text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}>{t(`whatsapp.${item.title}`)}</button>)}</nav>
      <section><div className="flex flex-wrap items-center justify-between gap-3"><h2 className="font-black text-gray-900">{t(`whatsapp.${section.title}`)}</h2><div className="flex overflow-hidden rounded-xl border"><button type="button" onClick={() => setLanguage('rw')} className={`min-h-11 px-4 text-sm font-bold ${language === 'rw' ? 'bg-fcs-brand-strong text-white' : 'bg-white text-gray-600'}`}> RW</button><button type="button" onClick={() => setLanguage('en')} className={`min-h-11 px-4 text-sm font-bold ${language === 'en' ? 'bg-fcs-brand-strong text-white' : 'bg-white text-gray-600'}`}>EN</button></div></div>
