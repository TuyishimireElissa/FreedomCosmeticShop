...
export function AdminCustomers() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setPagination({ page: data.pagination?.page || customerPage, total: data.pagination?.total || 0, totalPages: Math.max(1, data.pagination?.totalPages || 1) })
    } catch (e) {
      console.error(e)
    } finally {
      setLoading(false)
  ...
  return (
    <div>
      ...
      <div className="mb-4 flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            {pagination.total} total · {stats.newThisWeek} new this week ·{" "}
      ...
      {pagination.totalPages > 1 && <nav className="mt-4 flex items-center justify-between" aria-label="Customer pages"><Button type="button" variant="outline" size="sm" onClick={() => setCustomerPage((page) => Math.max(1, page - 1))} disabled={customerPage <= 1}>Previous</Button><span className="text-xs text-muted-foreground">Page {pagination.page} of {pagination.totalPages}</span><Button type="button" variant="outline" size="sm" onClick={() => setCustomerPage((page) => Math.min(pagination.totalPages, page + 1))} disabled={customerPage >= pagination.totalPages}>Next</Button></nav>}
