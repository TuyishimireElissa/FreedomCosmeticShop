...
export function AdminLoginScreen({ onBack }: AdminLoginScreenProps) {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        const failure = new Error(data.error || "Invalid credentials") as Error & { status?: number }
      ...
      if (rememberDevice) localStorage.setItem('freedom-admin-remember', 'true')
      else localStorage.removeItem('freedom-admin-remember')
      setAttempts(0)
      toast({
        title: "Welcome, Admin! ",
      ...
      if (failure.status === 423 || failure.status === 429 || (failure.status && failure.status >= 500)) {
        setError(failure.message || 'Login is temporarily unavailable.')
        return
      }
      const newAttempts = attempts + 1
      setAttempts(newAttempts)
      if (newAttempts >= MAX_ATTEMPTS) {
        ...
        setLockedUntil(lockTime)
        localStorage.setItem(LOCK_STORAGE_KEY, String(lockTime))
        setError('Too many failed attempts. This device is paused for 30 minutes.')
      } else {
        setError(failure.message || `Invalid credentials. ${MAX_ATTEMPTS - newAttempts} attempt${MAX_ATTEMPTS - newAttempts !== 1 ? 's' : ''} remaining.`)
      }
    } finally {
      setLoading(false)
    }
    ...
    return (
      <div className="flex min-h-screen flex-col items-center justify-center bg-background px-4">
        <div className="w-full max-w-md rounded-2xl border bg-card p-8 text-center shadow-lg">
          ...
          <h1 className="text-xl font-bold text-red-900">Login temporarily paused</h1>
