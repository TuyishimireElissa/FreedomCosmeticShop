...
const menuGroups: Array<{ group: string; items: MenuItem[] }> = [
  {
    group: 'Main',
    items: [{ label: 'Overview', tab: 'overview', icon: LayoutDashboard, permission: 'orders.read' }],
  },
  {
    group: 'Commerce',
    items: [
      ...
      { label: 'WhatsApp Pricing', tab: 'products', icon: MessageCircle, permission: 'products.update', href: '/admin/products/whatsapp-pricing', translationKey: 'pricing.nav' },
      ...
      { label: 'Deliveries', tab: 'deliveries', icon: Truck, permission: 'deliveries.read' },
      { label: 'Payments', tab: 'payments', icon: CreditCard, permission: 'orders.read' },
      { label: 'Wholesale', tab: 'wholesale', icon: Store, permission: 'wholesale.read' },
    ],
  },
  {
    group: 'Growth',
    items: [
      ...
      { label: 'Marketing', tab: 'marketing', icon: Megaphone, permission: 'coupons.read' },
      { label: 'SMS Campaigns', tab: 'sms', icon: MessageSquare, permission: 'sms.send' },
    ],
  },
  {
    group: 'System',
      ...
      >
        <div className="flex h-[72px] shrink-0 items-center border-b border-white/10 px-4">
          <button type="button" onClick={() => selectTab('overview')} className={`flex min-w-0 items-center gap-3 ${collapsed ? 'md:mx-auto' : ''}`} aria-label="Admin overview">
            <Logo size="sm" label="" className="shrink-0 rounded-xl bg-white p-1 shadow-lg shadow-black/25" />
            ...
            <div key={group.group} className="mb-3">
              ...
              <div className="space-y-1">
                {group.items.map((item) => {
                  ...
                  if (!canAdminPermission(user?.role, user?.permissions, item.permission)) return null
