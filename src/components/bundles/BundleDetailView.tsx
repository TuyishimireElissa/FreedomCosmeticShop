...
export default function BundleDetailView({ slug }: { slug: string }) {
  ...
  if (loading) return <div className="mx-auto flex min-h-[60vh] max-w-3xl items-center justify-center"><Loader2 className="h-7 w-7 animate-spin text-fcs-brand-text" /></div>
  if (error || !bundle) return <div className="mx-auto max-w-2xl px-4 py-16 text-center"><p className="font-bold text-gray-800">{t('bundles.not_found')}</p><Link href="/bundles" className="mt-4 inline-flex min-h-12 items-center rounded-xl bg-fcs-brand-strong px-5 font-bold text-white">{t('bundles.all_bundles')}</Link></div>
  ...
  return <main className="mx-auto max-w-4xl px-4 py-6 sm:px-6">
    ...
    <section className="mt-8"><h2 className="flex items-center gap-2 text-lg font-black"><Package className="h-5 w-5 text-fcs-brand-text" />{t('bundles.products_included', { count: bundle.products.length })}</h2><div className="mt-4 space-y-3">{bundle.products.map((item, index) => { const image = item.product.productImages?.[0]; const imageUrl = image?.publicId ? getCloudinaryUrl(image.publicId, 'THUMBNAIL') : image?.url || item.product.images?.[0]; return <article key={item.id} className={`flex items-center gap-3 rounded-2xl border p-3 ${item.isInStock ? 'border-gray-100 bg-white' : 'border-red-200 bg-red-50'}`}><span className="grid h-8 w-8 shrink-0 place-items-center rounded-full bg-fcs-brand-strong text-sm font-bold text-white">{item.stepOrder || index + 1}</span><span className="relative h-16 w-16 shrink-0 overflow-hidden rounded-xl bg-gray-50">{imageUrl && <Image src={imageUrl} alt={language === 'rw' && image?.altTextRw ? image.altTextRw : image?.altText || item.product.name} fill sizes="64px" loading="lazy" className="object-contain p-1" />}</span><div className="min-w-0 flex-1"><p className="text-xs font-bold text-fcs-brand-text">{language === 'rw' && item.stepLabelRw ? item.stepLabelRw : item.stepLabel}</p><Link href={`/products/${item.product.slug}`} className="line-clamp-2 text-sm font-bold text-gray-900">{item.product.name}</Link><p className="text-xs text-gray-500">{item.quantity > 1 ? `${item.quantity} × ` : ''}{formatRWF(item.product.price)}</p>{item.isOptional && <p className="text-xs italic text-fcs-text-muted">{t('bundles.optional')}</p>}</div><span className={`flex shrink-0 items-center gap-1 text-xs font-bold ${item.isInStock ? 'text-emerald-700' : 'text-red-600'}`}>{item.isInStock ? <Check className="h-3.5 w-3.5" /> : <AlertCircle className="h-3.5 w-3.5" />}{item.isInStock ? t('bundles.in_stock') : t('common.sold_out')}</span></article>})}</div></section>
    ...
    <button type="button" onClick={add} disabled={!bundle.isInStock} className="sticky bottom-[calc(68px+env(safe-area-inset-bottom))] mt-6 flex min-h-14 w-full items-center justify-center gap-2 rounded-2xl bg-fcs-brand-strong px-5 text-base font-black text-white shadow-lg disabled:bg-gray-300 md:bottom-6"><ShoppingCart className="h-5 w-5" />{bundle.isInStock ? t('bundles.add_to_cart', { price: formatRWF(bundle.bundlePrice) }) : t('common.sold_out')}</button>
  </main>
}
