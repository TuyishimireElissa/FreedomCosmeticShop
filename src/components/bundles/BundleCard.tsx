...
export default function BundleCard({ bundle }: { bundle: BundleCardData }) {
  ...
  return <article className="w-[84vw] max-w-sm flex-none snap-start overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-sm md:w-auto md:max-w-none">
    <div className="relative h-44 bg-gray-50">
      ...
      <span className="absolute left-3 top-3 rounded-full bg-fcs-brand px-2.5 py-1 text-xs font-bold text-white">{t(`bundles.type_${bundle.bundleType.toLowerCase()}`)}</span>
    ...
    <div className="p-4"><h2 className="font-black text-gray-900">{name}</h2>{description && <p className="mt-1 line-clamp-2 text-sm leading-6 text-gray-500">{description}</p>}
      ...
      <Link href={`/bundles/${bundle.slug}`} className={`mt-4 flex min-h-12 items-center justify-center gap-2 rounded-xl text-sm font-bold ${bundle.isInStock ? 'bg-fcs-brand text-white' : 'bg-gray-200 text-gray-600'}`}><Package className="h-4 w-4" />{t('bundles.view_bundle')}</Link>
    </div>
  </article>
}
