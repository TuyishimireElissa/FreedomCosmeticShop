...
export default function BundleCard({ bundle }: { bundle: BundleCardData }) {
  ...
  return <article className="w-[84vw] max-w-sm flex-none snap-start overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-sm md:w-auto md:max-w-none">
    <div className="relative h-44 bg-gray-50">
      ...
      <span className="absolute left-3 top-3 rounded-full bg-fcs-brand-strong px-2.5 py-1 text-xs font-bold text-white">{t(`bundles.type_${bundle.bundleType.toLowerCase()}`)}</span>
    ...
    <div className="p-4"><h2 className="font-black text-gray-900">{name}</h2>{description && <p className="mt-1 line-clamp-2 text-sm leading-6 text-gray-500">{description}</p>}
      <div className="mt-3 space-y-1">{bundle.products.slice(0, 4).map((item, index) => <p key={item.id} className="flex items-center gap-2 text-xs text-gray-600"><span className="grid h-5 w-5 place-items-center rounded-full bg-rose-50 font-bold text-fcs-brand-text">{item.stepOrder || index + 1}</span><span className="truncate">{language === 'rw' && item.stepLabelRw ? item.stepLabelRw : item.stepLabel || item.product.name}</span>{!item.isInStock && <AlertCircle className="ml-auto h-3.5 w-3.5 text-red-500" />}</p>)}</div>
      <div className="mt-4 rounded-xl bg-gray-50 p-3 text-sm"><p className="flex justify-between text-gray-500"><span>{t('bundles.normal_total')}</span><span>{formatRWF(bundle.normalTotal)}</span></p>{bundle.savings > 0 ? <p className="mt-1 flex justify-between font-bold text-emerald-700"><span>{t('bundles.you_save')}</span><span>{formatRWF(bundle.savings)} ({bundle.savingsPercent}%)</span></p> : bundle.savings < 0 ? <p className="mt-1 flex justify-between font-bold text-amber-700"><span>{t('bundles.additional_cost')}</span><span>{formatRWF(Math.abs(bundle.savings))}</span></p> : null}<p className="mt-2 flex items-center justify-between border-t border-gray-200 pt-2 font-black"><span>{t('bundles.bundle_price')}</span><span className="text-lg text-fcs-brand-text">{formatRWF(bundle.bundlePrice)}</span></p></div>
      ...
      <Link href={`/bundles/${bundle.slug}`} className={`mt-4 flex min-h-12 items-center justify-center gap-2 rounded-xl text-sm font-bold ${bundle.isInStock ? 'bg-fcs-brand-strong text-white' : 'bg-gray-200 text-gray-600'}`}><Package className="h-4 w-4" />{t('bundles.view_bundle')}</Link>
    </div>
  </article>
}
