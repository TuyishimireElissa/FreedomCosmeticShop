...
export default function BundlesView() {
  ...
  return <main className="mx-auto max-w-7xl px-4 py-7 sm:px-6 lg:px-8">
    ...
    <section className="mt-7 rounded-3xl bg-gradient-to-br from-fcs-brand to-[#73434b] p-5 text-white sm:flex sm:items-center sm:justify-between sm:gap-5 sm:p-7"><div><h2 className="flex items-center gap-2 text-xl font-black"><Sparkles className="h-5 w-5" />{t('bundles.quiz_title')}</h2><p className="mt-2 text-sm leading-6 text-white/80">{t('bundles.quiz_subtitle')}</p></div><Link href="/quiz" className="mt-4 flex min-h-12 items-center justify-center rounded-xl bg-white px-6 text-sm font-black text-fcs-brand-text sm:mt-0 sm:shrink-0">{t('bundles.quiz_cta')}</Link></section>

    <nav className="scrollbar-hide mt-7 flex gap-2 overflow-x-auto pb-2" aria-label={t('bundles.filter_label')}>{TYPES.map((type) => <button key={type || 'all'} type="button" onClick={() => setActiveType(type)} className={`min-h-11 shrink-0 rounded-full px-4 text-sm font-bold ${activeType === type ? 'bg-fcs-brand-strong text-white' : 'bg-gray-100 text-gray-700'}`}>{type ? t(`bundles.type_${type.toLowerCase()}`) : t('bundles.filter_all')}</button>)}</nav>

    {loading ? <div className="mt-8 flex items-center justify-center gap-2 py-16 text-sm text-gray-500"><Loader2 className="h-5 w-5 animate-spin text-fcs-brand-text" />{t('common.loading')}</div>
