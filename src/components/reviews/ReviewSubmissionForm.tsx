...
export default function ReviewSubmissionForm({ orderId, productId }: Props) {
  ...
  if (submitted) return <main className="mx-auto grid min-h-[60vh] max-w-xl place-items-center px-4 text-center"><div><span className="mx-auto grid h-20 w-20 place-items-center rounded-full bg-green-50 text-green-600"><Check className="h-9 w-9" /></span><h1 className="mt-5 text-2xl font-black">{t('reviews.thank_you')}</h1><p className="mt-2 text-sm text-gray-600">{t('reviews.points_awarded_equal', { points: REVIEW_REWARD_POINTS })}</p><Link href={`/products/${eligibility.product.slug}`} className="mt-6 inline-flex min-h-12 items-center rounded-xl bg-fcs-brand px-6 font-black text-white">{t('reviews.back_to_product')}</Link></div></main>
  ...
  return <main className="mx-auto max-w-2xl px-4 py-7 pb-28"><header><h1 className="text-3xl font-black text-gray-900">{t('reviews.form_title')}</h1><p className="mt-2 text-sm leading-6 text-gray-600">{t('reviews.equal_reward_notice', { points: REVIEW_REWARD_POINTS })}</p></header>
    ...
    <button type="button" onClick={submit} disabled={!rating || submitting || uploading} className="mt-7 flex min-h-[52px] w-full items-center justify-center rounded-xl bg-fcs-brand px-5 font-black text-white disabled:opacity-40">{submitting ? t('reviews.submitting') : t('reviews.submit_equal', { points: REVIEW_REWARD_POINTS })}</button><p className="mt-2 text-center text-xs text-gray-500">{t('reviews.negative_welcome')}</p>
  </main>
}
