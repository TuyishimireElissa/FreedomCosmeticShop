...
export default function ReviewSubmissionForm({ orderId, productId }: Props) {
  ...
  return <main className="mx-auto max-w-2xl px-4 py-7 pb-28"><header><h1 className="text-3xl font-black text-gray-900">{t('reviews.form_title')}</h1><p className="mt-2 text-sm leading-6 text-gray-600">{t('reviews.equal_reward_notice', { points: REVIEW_REWARD_POINTS })}</p></header>
    ...
    <section className="mt-6"><h2 className="text-sm font-black text-gray-900">{t('reviews.rating_label')}</h2><div className="mt-2 flex gap-1">{[1,2,3,4,5].map((star) => <button type="button" key={star} onClick={() => setRating(star)} className="grid h-12 w-12 place-items-center rounded-xl hover:bg-yellow-50" aria-label={t('reviews.stars_label', { count: star })}><Star className={`h-8 w-8 ${star <= rating ? 'fill-yellow-400 text-yellow-400' : 'fill-gray-100 text-gray-300'}`} /></button>)}</div>{rating > 0 && <p className="mt-2 text-sm font-bold text-gray-600">{t(`reviews.rating_${rating}`)}</p>}</section>

    <div className="mt-6 space-y-5"><label className="block"><span className="text-sm font-black text-gray-800">{t('reviews.title_optional')}</span><input value={title} onChange={(event) => setTitle(event.target.value)} maxLength={200} placeholder={t('reviews.title_placeholder')} className="input-field mt-2 min-h-12 text-base" /></label><label className="block"><span className="text-sm font-black text-gray-800">{t('reviews.comment_optional')}</span><textarea value={comment} onChange={(event) => setComment(event.target.value)} maxLength={2000} rows={5} placeholder={t('reviews.comment_placeholder')} className="input-field mt-2 resize-none text-base" /><span className="mt-1 block text-right text-xs text-gray-400">{comment.length}/2000</span></label></div>
    ...
    {error && <p className="mt-5 flex gap-2 rounded-xl bg-red-50 p-3 text-sm font-bold text-red-700"><AlertCircle className="h-5 w-5 shrink-0" />{reviewError(error, t)}</p>}
