...
export function HeroBanner({ banners, loading = false, error, onRetry }: HeroBannerProps) {
  ...
  if (loading) {
    return (
      <section className="relative min-h-[520px] overflow-hidden bg-[#f4e8ea] sm:min-h-[570px] lg:min-h-[640px]" aria-label={t('home.loading_promotions')}>
    ...
    return (
      <section className="relative overflow-hidden bg-gradient-to-br from-[#251d1f] via-[#4b3035] to-[#B76E79] px-4 py-24 text-white sm:px-6 sm:py-28 lg:px-8">
        ...
        <div className="relative mx-auto max-w-4xl text-center">
          ...
          <h1 className="mt-6 text-4xl font-black leading-tight sm:text-5xl lg:text-6xl">{t('home.hero_title')}</h1>
            ...
            <article key={banner.id} className={`absolute inset-0 transition-all duration-700 ${active ? 'z-10 opacity-100' : 'pointer-events-none z-0 opacity-0'}`} aria-hidden={!active}>
              ...
              <div className="relative z-10 mx-auto flex h-full max-w-7xl items-center px-4 sm:px-6 lg:px-8">
                <div className="max-w-2xl pt-6 text-white">
                  <span className="inline-flex items-center gap-2 rounded-full border border-white/15 bg-black/20 px-3 py-1.5 text-[10px] font-bold uppercase tracking-[0.18em] backdrop-blur sm:text-xs"><Sparkles className="h-3.5 w-3.5 text-[#FFD700]" />{t('home.premium_for_rwanda')}</span>
