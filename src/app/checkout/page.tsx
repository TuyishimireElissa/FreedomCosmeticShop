...
export default function CheckoutPage() {
  ...
  const [paymentAvailability, setPaymentAvailability] = useState({ mobileMoney: false, card: false })
  const [paymentPhone, setPaymentPhone] = useState(address.phone)
  const [deliveryFee, setDeliveryFee] = useState(1000)
  const [deliveryLoading, setDeliveryLoading] = useState(false)
  const [placing, setPlacing] = useState(false)
  const [checkoutError, setCheckoutError] = useState<string | null>(null)
  const [pendingOrder, setPendingOrder] = useState<{ id: string; orderNumber: string; total: number; orderAccessToken: string } | null>(null)
  ...
  useEffect(() => {
    fetch('/api/config/features')
      ...
      .then((data) => setPaymentAvailability({ mobileMoney: data.payments?.mobileMoney === true, card: data.payments?.card === true }))
      .catch(() => setPaymentAvailability({ mobileMoney: false, card: false }))
  }, [])
  useEffect(() => {
  ...
  useEffect(() => {
    if (!paymentAvailability.mobileMoney && !paymentAvailability.card && address.province === 'Kigali City') setPaymentMethod('COD')
    else if (paymentMethod === 'COD' && address.province !== 'Kigali City') setPaymentMethod(paymentAvailability.mobileMoney ? 'MTN_MOMO' : paymentAvailability.card ? 'CARD' : 'MTN_MOMO')
  }, [address.province, paymentAvailability, paymentMethod])
  useEffect(() => {
  ...
  const methodAvailable = paymentMethod === 'COD' ? address.province === 'Kigali City' : paymentMethod === 'CARD' ? paymentAvailability.card : paymentAvailability.mobileMoney
  const momoValid = paymentMethod === 'MTN_MOMO' ? paymentAvailability.mobileMoney && /^(?:250|0)?7[89]\d{7}$/.test(paymentPhone.replace(/\D/g, '')) : paymentMethod === 'AIRTEL_MONEY' ? paymentAvailability.mobileMoney && /^(?:250|0)?7[23]\d{7}$/.test(paymentPhone.replace(/\D/g, '')) : paymentMethod === 'CARD' ? paymentAvailability.card && /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(address.email) : methodAvailable

  const finishOrder = (order: { id: string; orderNumber: string; total: number; orderAccessToken: string }) => {
  ...
  const initiatePayment = async (order: { id: string; orderNumber: string; total: number; orderAccessToken: string }) => {
    trackPaymentStarted(paymentMethod, order.total)
    try {
      if (paymentMethod === 'COD') { finishOrder(order); return }
    if (paymentMethod === 'CARD') {
      const response = await fetch('/api/payments/card', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ orderId: order.id, language, orderAccessToken: order.orderAccessToken }) })
      ...
      polling.start(data.transactionId, order.orderAccessToken); return
    }
    const network = paymentMethod === 'MTN_MOMO' ? 'MTN' : 'AIRTEL'
    const response = await fetch('/api/payments/momo', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ orderId: order.id, phone: paymentPhone, network, language, orderAccessToken: order.orderAccessToken }) })
    const data = await response.json(); if (!response.ok) throw new Error(data.error === 'ORDER_STOCK_CHANGED' ? t('checkout.stock_changed_before_payment') : data.error === 'INVALID_NETWORK_PHONE' ? t('checkout.invalid_network_phone', { network }) : t('checkout.mobile_money_start_failed'))
      polling.start(data.transactionId, order.orderAccessToken)
    } catch (error) {
  ...
  const placeOrder = async () => {
    ...
    try {
      ...
      const order = { id: orderData.order.id, orderNumber: orderData.order.orderNumber, total: orderData.order.total, orderAccessToken: orderData.orderAccessToken || orderData.order.orderAccessToken, confirmationDelivery: orderData.confirmationDelivery }
  ...
  if (items.length === 0 && !completedOrder) return <div className="mx-auto grid min-h-[65vh] max-w-2xl place-items-center px-4 text-center"><div><ShoppingBag className="mx-auto h-14 w-14 text-fcs-brand-text" /><h1 className="mt-5 text-2xl font-bold">{t('cart.empty')}</h1><p className="mt-2 text-sm text-gray-500">{t('empty.cart_hint')}</p><Link href="/products" className="mt-5 inline-flex min-h-12 items-center rounded-full bg-fcs-brand px-6 py-3 text-base font-bold text-white">{t('nav.products')}</Link></div></div>

  return (
    <div className="safe-bottom min-h-screen bg-[#FAFAFA]">
      <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6 lg:px-8">
        <div className="mb-7 text-center"><span className="mb-3 inline-flex items-center justify-center gap-2"><BrandMark variant="badge" size={24} className="h-6 w-6 object-contain" /><span className="text-sm font-bold text-[#1a1a1a]">FreedomCosmeticShop</span></span><br /><span className="text-xs font-bold uppercase tracking-[0.2em] text-fcs-brand-text">{t('checkout.secure_rwanda_checkout')}</span><h1 className="mt-2 text-3xl font-bold text-[#1a1a1a]">{t('checkout.title')}</h1>{wholesaleShoppingMode && <p className="mx-auto mt-3 max-w-xl rounded-xl bg-amber-50 px-4 py-3 text-sm font-bold text-amber-900">{t('checkout.wholesale_mode_active')}</p>}</div>
        <div className="mb-6 flex items-center gap-1 lg:hidden" aria-label={t('checkout.title')}>
          {[1, 2, 3].map((number) => <span key={number} className={`h-1.5 flex-1 rounded-full transition-colors duration-300 ${number <= step ? 'bg-fcs-brand' : 'bg-gray-200'}`} />)}
        </div>
        <div className="mx-auto mb-8 hidden max-w-xl items-center lg:flex">{([{ label: t('checkout.step_address'), number: 1 }, { label: t('checkout.step_payment'), number: 2 }, { label: t('checkout.step_confirm'), number: 3 }] as const).map(({ label, number }, index) => <div key={label} className="contents"><div className="flex flex-col items-center gap-1"><span className={`grid h-9 w-9 place-items-center rounded-full text-xs font-bold ${step >= number ? 'bg-fcs-brand text-white' : 'bg-gray-200 text-gray-500'}`}>{step > number ? <Check className="h-4 w-4" /> : number}</span><span className="text-sm font-bold text-gray-500">{label}</span></div>{index < 2 && <div className={`mb-5 h-0.5 flex-1 ${step > number ? 'bg-fcs-brand' : 'bg-gray-200'}`} />}</div>)}</div>
        ...
        {step === 3 && completedOrder ? <ConfirmationView order={completedOrder} address={address} paymentMethod={paymentMethod} /> : <div className="grid items-start gap-6 lg:grid-cols-[1fr_340px]">
          ...
          <details className="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm lg:hidden">
            <summary className="flex min-h-[52px] cursor-pointer list-none items-center justify-between gap-3 p-4 text-sm font-semibold text-gray-800">
              ...
              <span className="shrink-0 font-bold text-fcs-brand-text">{formatRWF(total)}</span>
            </summary>
