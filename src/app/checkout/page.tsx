...
export default function CheckoutPage() {
  ...
  if (items.length === 0 && !completedOrder) return <div className="mx-auto grid min-h-[65vh] max-w-2xl place-items-center px-4 text-center"><div><ShoppingBag className="mx-auto h-14 w-14 text-[#B76E79]" /><h1 className="mt-5 text-2xl font-black">{t('cart.empty')}</h1><p className="mt-2 text-sm text-gray-500">{t('empty.cart_hint')}</p><Link href="/products" className="mt-5 inline-block rounded-full bg-[#B76E79] px-6 py-3 text-sm font-bold text-white">{t('nav.products')}</Link></div></div>

  return (
    <div className="min-h-screen bg-[#f8f9fa]">
      <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6 lg:px-8">
        <div className="mb-7 text-center"><span className="text-[10px] font-black uppercase tracking-[0.2em] text-[#B76E79]">{t('checkout.secure_rwanda_checkout')}</span><h1 className="mt-2 text-3xl font-black text-[#1a1a1a]">{t('checkout.title')}</h1></div>
        <div className="mx-auto mb-8 flex max-w-xl items-center">{([{ label: t('checkout.step_address'), number: 1 }, { label: t('checkout.step_payment'), number: 2 }, { label: t('checkout.step_confirm'), number: 3 }] as const).map(({ label, number }, index) => <div key={label} className="contents"><div className="flex flex-col items-center gap-1"><span className={`grid h-9 w-9 place-items-center rounded-full text-xs font-black ${step >= number ? 'bg-[#B76E79] text-white' : 'bg-gray-200 text-gray-500'}`}>{step > number ? <Check className="h-4 w-4" /> : number}</span><span className="text-[10px] font-bold text-gray-500 sm:text-xs">{label}</span></div>{index < 2 && <div className={`mb-5 h-0.5 flex-1 ${step > number ? 'bg-[#B76E79]' : 'bg-gray-200'}`} />}</div>)}</div>
        ...
        {step === 3 && completedOrder ? <Confirmation order={completedOrder} address={address} paymentMethod={paymentMethod} /> : <div className="grid items-start gap-6 lg:grid-cols-[1fr_340px]">
          <main className="rounded-3xl border border-gray-100 bg-white p-5 shadow-sm sm:p-7">
            {step === 1 ? <><div className="mb-6"><h2 className="flex items-center gap-2 text-xl font-black"><Truck className="h-5 w-5 text-[#B76E79]" />{t('checkout.step_address')}</h2><p className="mt-1 text-sm text-gray-500">{t('checkout.delivery_intro')}</p></div><AddressForm value={address} onChange={setAddress} errors={addressErrors} /><div className="mt-5 flex items-center justify-between rounded-2xl bg-rose-50 p-4"><span><strong className="block text-sm text-gray-800">{t('checkout.delivery_to', { place: address.district })}</strong><span className="text-xs text-gray-500">{deliveryLoading ? t('checkout.calculating') : address.province}</span></span><strong className="text-[#B76E79]">{deliveryLoading ? '—' : formatRWF(finalDelivery)}</strong></div><button type="button" onClick={() => { if (addressValid) { setStep(2); window.scrollTo({ top: 0, behavior: 'smooth' }) } }} disabled={!addressValid} className="mt-6 flex min-h-12 w-full items-center justify-center gap-2 rounded-xl bg-[#B76E79] text-sm font-black text-white disabled:opacity-40">{t('checkout.step_payment')} <ChevronRight className="h-4 w-4" /></button></> : <><div className="mb-6"><h2 className="text-xl font-black">{t('checkout.choose_payment')}</h2><p className="mt-1 text-sm text-gray-500">{t('checkout.mtn_fastest')}</p></div><PaymentSelector method={paymentMethod} onMethodChange={setPaymentMethod} phone={paymentPhone} onPhoneChange={setPaymentPhone} isKigali={address.province === 'Kigali City'} paymentStatus={placing ? 'initiating' : polling.status} paymentError={checkoutError || polling.error} total={formatRWF(total)} />{!momoValid && <p className="mt-3 text-xs font-bold text-red-600">{t('checkout.valid_network_number', { network: paymentMethod === 'MTN_MOMO' ? 'MTN (078/079)' : 'Airtel (072/073)' })}</p>}{checkoutError && <p className="mt-4 rounded-xl bg-red-50 p-3 text-sm font-semibold text-red-700">{checkoutError}</p>}<div className="mt-6 flex gap-3"><button type="button" onClick={() => setStep(1)} disabled={placing || polling.status === 'polling'} className="min-h-12 flex-1 rounded-xl border border-gray-200 text-sm font-bold">{t('common.back')}</button><button type="button" onClick={placeOrder} disabled={placing || polling.status === 'polling' || !momoValid} className={`min-h-12 flex-[2] rounded-xl px-4 text-sm font-black shadow-lg disabled:opacity-50 ${paymentMethod === 'MTN_MOMO' ? 'bg-[#FFD200] text-[#1a1a1a] shadow-yellow-200' : 'bg-[#B76E79] text-white shadow-rose-100'}`}>{placing || polling.status === 'polling' ? <span className="flex items-center justify-center gap-2"><Loader2 className="h-4 w-4 animate-spin" />{polling.status === 'polling' ? t('checkout.approve_on_phone') : t('checkout.creating_order')}</span> : paymentMethod === 'COD' ? t('checkout.place_order_amount', { amount: formatRWF(total) }) : t('checkout.pay_amount', { amount: formatRWF(total) })}</button></div><p className="mt-4 flex items-center justify-center gap-1.5 text-[10px] font-semibold text-gray-400"><ShieldCheck className="h-3.5 w-3.5 text-emerald-600" />{t('checkout.rwanda_payment_security')}</p></>}
          </main>
