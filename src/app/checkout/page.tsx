...
export default function CheckoutPage() {
  ...
  if (items.length === 0 && !completedOrder) return <div className="mx-auto grid min-h-[65vh] max-w-2xl place-items-center px-4 text-center"><div><ShoppingBag className="mx-auto h-14 w-14 text-fcs-brand-text" /><h1 className="mt-5 text-2xl font-bold">{t('cart.empty')}</h1><p className="mt-2 text-sm text-gray-500">{t('empty.cart_hint')}</p><Link href="/products" className="mt-5 inline-flex min-h-12 items-center rounded-full bg-fcs-brand-strong px-6 py-3 text-base font-bold text-white">{t('nav.products')}</Link></div></div>
  ...
  return (
    <div className="safe-bottom min-h-screen bg-[#FAFAFA]">
      <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        <div className="mx-auto mb-8 hidden max-w-xl items-center lg:flex">{([{ label: t('checkout.step_address'), number: 1 }, { label: t('checkout.step_payment'), number: 2 }, { label: t('checkout.step_confirm'), number: 3 }] as const).map(({ label, number }, index) => <div key={label} className="contents"><div className="flex flex-col items-center gap-1"><span className={`grid h-9 w-9 place-items-center rounded-full text-xs font-bold ${step >= number ? 'bg-fcs-brand-strong text-white' : 'bg-gray-200 text-gray-500'}`}>{step > number ? <Check className="h-4 w-4" /> : number}</span><span className="text-sm font-bold text-gray-500">{label}</span></div>{index < 2 && <div className={`mb-5 h-0.5 flex-1 ${step > number ? 'bg-fcs-brand' : 'bg-gray-200'}`} />}</div>)}</div>
