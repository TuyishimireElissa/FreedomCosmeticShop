...
interface Result {
  status: string
  order?: { id: string; orderNumber: string; total: number; orderAccessToken?: string }
...
function PaymentResult() {
  ...
  useEffect(() => {
    ...
    void check()
    return () => { stopped = true }
  }, [accessToken, clearCart, paymentId, trackPaymentFailed, trackPurchaseCompleted])
...
function Loading() { const t = useT(); return <main className="grid min-h-[65vh] place-items-center bg-[#f8f9fa] px-4"><div className="text-center"><Loader2 className="mx-auto h-10 w-10 animate-spin text-fcs-brand-text" /><h1 className="mt-4 text-xl font-black">{t('confirmation.confirming_payment')}</h1><p className="mt-2 text-sm text-gray-500">{t('confirmation.keep_open')}</p></div></main> }
