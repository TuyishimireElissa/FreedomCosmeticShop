  ...
  return (
    <main className="grid min-h-dvh bg-white lg:grid-cols-[0.9fr_1.1fr]">
      <section className="relative hidden overflow-hidden bg-[#1a1a1a] p-12 text-white lg:flex lg:flex-col lg:justify-between">
        ...
        <Link href="/" className="relative flex items-center gap-3"><BrandMark size={48} priority className="h-12 w-12 rounded-full bg-white object-contain p-1.5" /><span><strong className="block text-lg">FreedomCosmeticShop</strong><span className="text-xs text-[#d999a3]">{t('auth.rwanda_beauty_freedom')}</span></span></Link>
        <div className="relative max-w-xl"><span className="text-xs font-black uppercase tracking-[0.22em] text-[#FFD700]">{t('auth.login_title')}</span><h1 className="mt-5 text-5xl font-black leading-tight">{t('auth.favourites_waiting')}</h1><p className="mt-5 max-w-lg text-base leading-7 text-fcs-text-muted">{t('auth.signin_benefits')}</p><div className="mt-8 grid gap-3 sm:grid-cols-2">{[t('auth.benefit_authentic'), t('auth.benefit_delivery'), t('auth.benefit_momo'), t('auth.benefit_secure')].map((item) => <span key={item} className="flex items-center gap-2 text-sm text-gray-300"><CheckCircle2 className="h-4 w-4 text-emerald-400" />{item}</span>)}</div></div>
      ...
      <section className="flex items-center justify-center bg-[#FAFAFA] px-4 py-10 sm:px-8">
        <div className="w-full max-w-md">
          <Link href="/" className="mb-8 flex min-h-11 items-center justify-center gap-3 lg:hidden"><BrandMark size={44} priority className="h-11 w-11 rounded-full object-contain" /><span><strong className="block text-base text-[#1a1a1a]">FreedomCosmeticShop</strong><span className="text-xs font-semibold text-fcs-brand-text">{t('auth.rwanda_beauty_freedom')}</span></span></Link>
          <div className="rounded-xl border border-[#EEEEEE] bg-white p-6 shadow-[0_1px_3px_rgba(0,0,0,0.04)] sm:p-8">
            ...
            <span className="mx-auto grid h-12 w-12 place-items-center rounded-2xl bg-rose-50 text-fcs-brand-text"><User className="h-5 w-5" /></span><h2 className="mt-5 text-center text-3xl font-black text-[#1a1a1a]">{t('auth.login_title')}</h2><p className="mt-2 text-center text-sm text-gray-500">{t('auth.login_rwanda_identifier')}</p>
            <form onSubmit={submit} className="mt-6 space-y-4">
              ...
              <FormField
                ...
                labelExtra={<Link href="/forgot-password" className="inline-flex min-h-11 items-center text-sm font-bold text-fcs-brand-text hover:underline">{t('auth.forgot_password')}</Link>}
              ...
              />
              <button type="submit" disabled={loading} className="flex min-h-12 w-full items-center justify-center gap-2 rounded-[10px] bg-fcs-brand-strong text-base font-black text-white  hover:bg-[#9B5A64] disabled:opacity-50">{loading ? <><Loader2 className="h-4 w-4 animate-spin" />{t('auth.logging_in')}</> : <>{t('auth.login_button')} <ArrowRight className="h-4 w-4" /></>}</button>
            </form>
            <p className="mt-5 text-center text-sm text-gray-500">{t('auth.new_customer')} <Link href="/register" className="inline-flex min-h-11 items-center font-black text-fcs-brand-text hover:underline">{t('auth.register_link')}</Link></p><p className="mt-5 flex items-center justify-center gap-1.5 text-xs font-semibold text-gray-500"><ShieldCheck className="h-3.5 w-3.5 text-emerald-600" />{t('auth.secure_login')}</p>
            </>}
          </div>
