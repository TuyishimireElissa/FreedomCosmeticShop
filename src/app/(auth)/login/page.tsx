  ...
  return (
    <main className="grid min-h-dvh bg-white lg:grid-cols-[0.9fr_1.1fr]">
      <section className="relative hidden overflow-hidden bg-[#1a1a1a] p-12 text-white lg:flex lg:flex-col lg:justify-between">
        ...
        <div className="relative max-w-xl"><span className="text-[11px] font-black uppercase tracking-[0.22em] text-[#FFD700]">{t('auth.login_title')}</span><h1 className="mt-5 text-5xl font-black leading-tight">{t('auth.favourites_waiting')}</h1><p className="mt-5 max-w-lg text-base leading-7 text-gray-400">{t('auth.signin_benefits')}</p><div className="mt-8 grid gap-3 sm:grid-cols-2">{[t('auth.benefit_authentic'), t('auth.benefit_delivery'), t('auth.benefit_momo'), t('auth.benefit_secure')].map((item) => <span key={item} className="flex items-center gap-2 text-sm text-gray-300"><CheckCircle2 className="h-4 w-4 text-emerald-400" />{item}</span>)}</div></div>
        ...
        <div className="w-full max-w-md">
          ...
          <div className="rounded-[2rem] border border-gray-100 bg-white p-6 shadow-[0_20px_60px_rgba(26,26,26,0.08)] sm:p-8">
            ...
            <form onSubmit={submit} className="mt-6 space-y-4">
              <label className="block"><span className="mb-1.5 block text-xs font-black uppercase tracking-wider text-gray-500">{t('auth.phone_or_email')}</span><input value={identifier} onChange={(event) => setIdentifier(event.target.value)} placeholder="+250780000000 or you@email.com" autoComplete="username" className="input-field" required /></label>
              <label className="block"><span className="mb-1.5 flex items-center justify-between text-xs font-black uppercase tracking-wider text-gray-500">{t('auth.password')}<Link href="/forgot-password" className="normal-case tracking-normal text-[#B76E79] hover:underline">{t('auth.forgot_password')}</Link></span><div className="relative"><Lock className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-gray-400" /><input type={showPassword ? 'text' : 'password'} value={password} onChange={(event) => setPassword(event.target.value)} placeholder={t('auth.password_placeholder')} autoComplete="current-password" className="input-field pl-10 pr-11" required /><button type="button" onClick={() => setShowPassword((value) => !value)} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400" aria-label={showPassword ? t('auth.hide_password') : t('auth.show_password')}>{showPassword ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}</button></div></label>
            ...
            <p className="mt-5 text-center text-sm text-gray-500">{t('auth.new_customer')} <Link href="/register" className="font-black text-[#B76E79] hover:underline">{t('auth.register_link')}</Link></p><p className="mt-5 flex items-center justify-center gap-1.5 text-[10px] font-semibold text-gray-400"><ShieldCheck className="h-3.5 w-3.5 text-emerald-600" />{t('auth.secure_login')}</p>
            </>}
          </div>
