    ...
    <main className="min-h-dvh bg-gradient-to-br from-rose-50 via-white to-[#fff8e7] px-4 py-8 sm:px-6">
      <div className="mx-auto max-w-2xl">
        ...
        <div className="overflow-hidden rounded-[2rem] border border-white bg-white shadow-[0_24px_70px_rgba(26,26,26,0.1)]">
          <div className="bg-[#1a1a1a] px-6 py-7 text-center text-white sm:px-9"><span className="mx-auto grid h-11 w-11 place-items-center rounded-2xl bg-fcs-brand"><Sparkles className="h-5 w-5" /></span><h1 className="mt-4 text-2xl font-black sm:text-3xl">{step === 'form' ? t('auth.register_title') : t('auth.otp_title')}</h1><p className="mt-2 text-sm text-fcs-text-muted">{step === 'form' ? t('auth.register_benefits') : t('auth.enter_code_phone', { phone: form.phone })}</p></div>
          <div className="p-5 sm:p-8">
            ...
            {step === 'form' ? <form onSubmit={submit} className="space-y-4">
              ...
              <fieldset><legend className="mb-1.5 text-sm font-semibold text-gray-900">{t('auth.skin_type')}</legend><div className="grid grid-cols-2 gap-2 sm:grid-cols-3">{skinTypes.map((type) => <button key={type} type="button" aria-pressed={form.skinType === type} onClick={() => update('skinType', type)} className={`min-h-11 rounded-xl border px-3 py-2.5 text-xs font-bold ${form.skinType === type ? 'border-fcs-brand bg-rose-50 text-fcs-brand-text' : 'border-gray-200 text-gray-600 hover:border-rose-200'}`}>{type === 'NOT_SURE' ? t('auth.not_sure') : t(`skin_types.${type}`)}</button>)}</div></fieldset>
              <div><label className="flex min-h-12 cursor-pointer items-start gap-3 rounded-xl bg-[#f8f9fa] p-3"><input required aria-required="true" aria-invalid={fieldErrors.terms ? true : undefined} aria-describedby={fieldErrors.terms ? 'register-terms-error' : undefined} type="checkbox" checked={form.terms} onChange={(event) => update('terms', event.target.checked)} className="mt-0.5 h-5 w-5 rounded border-gray-300 text-fcs-brand-text focus:ring-fcs-brand" /><span className="text-sm leading-6 text-gray-600">{t('auth.terms_agree')} <Link href="/terms" className="font-bold text-fcs-brand-text">{t('auth.terms_link')}</Link> {t('common.and')} <Link href="/privacy" className="font-bold text-fcs-brand-text">{t('auth.privacy_policy')}</Link>.</span></label>{fieldErrors.terms && <p id="register-terms-error" role="alert" className="mt-1 flex items-center gap-1.5 text-sm font-semibold text-red-700"><AlertCircle className="h-4 w-4 shrink-0" aria-hidden="true" />{fieldErrors.terms}</p>}</div>
              <button type="submit" disabled={loading} className="flex min-h-12 w-full items-center justify-center gap-2 rounded-[10px] bg-fcs-brand-strong text-base font-black text-white shadow-lg shadow-rose-100 disabled:opacity-50">{loading ? <><Loader2 className="h-4 w-4 animate-spin" />{t('auth.sending_otp')}</> : <>{t('auth.register_button')} <ArrowRight className="h-4 w-4" /></>}</button>
        ...
        <p className="mt-6 text-center text-sm text-gray-500">{t('auth.already_registered')} <Link href="/login" className="inline-flex min-h-11 items-center font-black text-fcs-brand-text">{t('auth.login_link')}</Link></p>
      </div>
    </main>
  )
}
