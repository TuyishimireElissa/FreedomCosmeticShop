        ...
        <div className="overflow-hidden rounded-[2rem] border border-white bg-white shadow-[0_24px_70px_rgba(26,26,26,0.1)]">
          ...
          <div className="p-5 sm:p-8">
            ...
            {step === 'form' ? <form onSubmit={submit} className="space-y-4">
              ...
              <button type="submit" disabled={loading} className="flex min-h-12 w-full items-center justify-center gap-2 rounded-[10px] bg-fcs-brand text-base font-black text-white shadow-lg shadow-rose-100 disabled:opacity-50">{loading ? <><Loader2 className="h-4 w-4 animate-spin" />{t('auth.sending_otp')}</> : <>{t('auth.register_button')} <ArrowRight className="h-4 w-4" /></>}</button>
