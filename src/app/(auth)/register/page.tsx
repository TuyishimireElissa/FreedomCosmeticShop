      ...
      <div className="mx-auto max-w-2xl">
        ...
        <div className="overflow-hidden rounded-[2rem] border border-white bg-white shadow-[0_24px_70px_rgba(26,26,26,0.1)]">
          ...
          <div className="p-5 sm:p-8">
            {error && <div role="alert" aria-live="polite" className="mb-5 rounded-xl bg-red-50 p-3 text-center text-sm font-semibold text-red-700">{error}</div>}
            {step === 'form' ? <form onSubmit={submit} className="space-y-4">
              <div className="grid gap-4 sm:grid-cols-2"><Field label={t('auth.full_name')}><input required value={form.name} onChange={(event) => update('name', event.target.value)} placeholder={t('auth.full_name_example')} autoComplete="name" className="input-field" /></Field><Field label={t('auth.rwanda_phone_label')}><input required type="tel" inputMode="tel" value={form.phone} onChange={(event) => update('phone', event.target.value)} placeholder={t('auth.phone_placeholder')} autoComplete="tel" className="input-field" /></Field></div>
              <Field label={t('auth.email_optional')}><input type="email" value={form.email} onChange={(event) => update('email', event.target.value)} placeholder={t('auth.email_placeholder')} autoComplete="email" className="input-field" /></Field>
              ...
              <Field label={t('auth.skin_type')}><div className="grid grid-cols-2 gap-2 sm:grid-cols-3">{skinTypes.map((type) => <button key={type} type="button" onClick={() => update('skinType', type)} className={`min-h-11 rounded-xl border px-3 py-2.5 text-xs font-bold ${form.skinType === type ? 'border-[#B76E79] bg-rose-50 text-[#B76E79]' : 'border-gray-200 text-gray-600 hover:border-rose-200'}`}>{type === 'NOT_SURE' ? t('auth.not_sure') : t(`skin_types.${type}`)}</button>)}</div></Field>
              <label className="flex min-h-12 cursor-pointer items-start gap-3 rounded-xl bg-[#f8f9fa] p-3"><input required type="checkbox" checked={form.terms} onChange={(event) => update('terms', event.target.checked)} className="mt-0.5 h-5 w-5 rounded border-gray-300 text-[#B76E79] focus:ring-[#B76E79]" /><span className="text-sm leading-6 text-gray-600">{t('auth.terms_agree')} <Link href="/terms" className="font-bold text-[#B76E79]">{t('auth.terms_link')}</Link> {t('common.and')} <Link href="/privacy" className="font-bold text-[#B76E79]">{t('auth.privacy_policy')}</Link>.</span></label>
...
function Password({ value, onChange, visible, onToggle }: { value: string; onChange: (value: string) => void; visible: boolean; onToggle: () => void }) { const t = useT(); return <div className="relative"><Lock className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-gray-400" /><input required type={visible ? 'text' : 'password'} value={value} onChange={(event) => onChange(event.target.value)} placeholder={t('auth.password_hint')} autoComplete="new-password" className="input-field pl-10 pr-10" /><button type="button" onClick={onToggle} className="absolute right-0 top-1/2 grid h-11 w-11 -translate-y-1/2 touch-manipulation place-items-center rounded-full text-gray-500">{visible ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}</button></div> }
