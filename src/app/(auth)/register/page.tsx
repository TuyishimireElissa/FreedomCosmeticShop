        ...
        <div className="overflow-hidden rounded-[2rem] border border-white bg-white shadow-[0_24px_70px_rgba(26,26,26,0.1)]">
          ...
          <div className="p-5 sm:p-8">
            ...
            {step === 'form' ? <form onSubmit={submit} className="space-y-4">
              ...
              <label className="flex cursor-pointer items-start gap-3 rounded-xl bg-[#f8f9fa] p-3"><input type="checkbox" checked={form.terms} onChange={(event) => update('terms', event.target.checked)} className="mt-0.5 h-5 w-5 rounded border-gray-300 text-[#B76E79] focus:ring-[#B76E79]" /><span className="text-xs leading-5 text-gray-600">{t('auth.terms_agree')} <Link href="/terms" className="font-bold text-[#B76E79]">{t('auth.terms_link')}</Link> {t('common.and')} <Link href="/privacy" className="font-bold text-[#B76E79]">{t('auth.privacy_policy')}</Link>.</span></label>
              <button type="submit" disabled={loading} className="flex min-h-12 w-full items-center justify-center gap-2 rounded-xl bg-[#B76E79] text-sm font-black text-white shadow-lg shadow-rose-100 disabled:opacity-50">{loading ? <><Loader2 className="h-4 w-4 animate-spin" />{t('auth.sending_otp')}</> : <>{t('auth.register_button')} <ArrowRight className="h-4 w-4" /></>}</button>
              <p className="flex items-center justify-center gap-1.5 text-[10px] font-semibold text-gray-400"><ShieldCheck className="h-3.5 w-3.5 text-emerald-600" />{t('auth.password_hashed')}</p>
            </form> : <div className="py-4 text-center"><button type="button" onClick={() => { setStep('form'); setOtp(''); setError(null) }} className="mb-6 inline-flex items-center gap-1 text-xs font-bold text-gray-500"><ArrowLeft className="h-3.5 w-3.5" />{t('auth.edit_details')}</button>{devCode && <div className="mb-5 rounded-xl border border-amber-200 bg-amber-50 p-3"><p className="text-[10px] font-bold uppercase tracking-wider text-amber-700">{t('auth.development_otp')}</p><p className="mt-1 text-xl font-black tracking-[0.3em] text-amber-900">{devCode}</p></div>}<OTPInput value={otp} onChange={setOtp} onComplete={verify} disabled={loading} />{loading && <p className="mt-4 flex items-center justify-center gap-2 text-sm text-gray-500"><Loader2 className="h-4 w-4 animate-spin" />{t('auth.verifying_account')}</p>}<p className="mt-6 text-xs text-gray-500">{t('auth.code_expires_return')}</p></div>}
          </div>
        </div>
        <p className="mt-6 text-center text-sm text-gray-500">{t('auth.already_registered')} <Link href="/login" className="font-black text-[#B76E79]">{t('auth.login_link')}</Link></p>
      </div>
    </main>
  )
}
