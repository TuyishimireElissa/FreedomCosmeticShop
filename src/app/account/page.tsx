  ...
  return (
    <main className="min-h-screen bg-[#f8f9fa] px-4 py-8 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-5xl">
        <div className="overflow-hidden rounded-3xl bg-[#1a1a1a] p-6 text-white shadow-xl sm:p-8"><div className="flex flex-col gap-5 sm:flex-row sm:items-center"><span className="grid h-20 w-20 shrink-0 place-items-center rounded-full bg-[#B76E79] text-2xl font-black">{user.name.split(' ').map((part) => part[0]).slice(0,2).join('').toUpperCase()}</span><div className="min-w-0"><span className="text-[10px] font-black uppercase tracking-[0.2em] text-[#FFD700]">{t('auth.my_beauty_account')}</span><h1 className="mt-2 truncate text-3xl font-black">{user.name}</h1><div className="mt-2 flex flex-wrap gap-3 text-xs text-gray-400"><span className="flex items-center gap-1"><Phone className="h-3.5 w-3.5" />{user.phone}</span>{user.email && <span className="flex items-center gap-1"><Mail className="h-3.5 w-3.5" />{user.email}</span>}</div></div></div></div>
...
function AuthRequired() { const t = useT(); return <div className="grid min-h-[60vh] place-items-center px-4 text-center"><div><User className="mx-auto h-12 w-12 text-[#B76E79]" /><h1 className="mt-4 text-2xl font-black">{t('auth.signin_account')}</h1><Link href="/login" className="mt-5 inline-block rounded-full bg-[#B76E79] px-6 py-3 text-sm font-bold text-white">{t('auth.login_button')}</Link></div></div> }
