...
export default function ProductDetailPage({ params }: { params: { slug: string } }) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/products/${encodeURIComponent(params.slug)}`, { signal: controller.signal, cache: 'no-store' })
      .then((response) => { if (!response.ok) throw new Error(response.status === 404 ? 'Product not found' : 'Product is unavailable'); return response.json() })
  ...
  if (error || !data) return <div className="mx-auto grid min-h-[60vh] max-w-3xl place-items-center px-4 py-16 text-center"><div><div className="mx-auto grid h-16 w-16 place-items-center rounded-full bg-rose-50 text-[#B76E79]"><ShoppingBag className="h-7 w-7" /></div><h1 className="mt-5 text-2xl font-black text-[#1a1a1a]">{error || 'Product not found'}</h1><p className="mt-2 text-sm text-gray-500">The product may have moved or is temporarily unavailable.</p><div className="mt-6 flex justify-center gap-3"><button type="button" onClick={() => router.push('/products')} className="rounded-full bg-[#1a1a1a] px-5 py-2.5 text-sm font-bold text-white">Browse products</button><button type="button" onClick={() => setRequest((value) => value + 1)} className="inline-flex items-center gap-2 rounded-full border border-gray-200 px-5 py-2.5 text-sm font-bold"><RefreshCw className="h-4 w-4" />Retry</button></div></div></div>
  ...
  return (
    <div className="bg-white">
      <div className="mx-auto max-w-7xl px-4 py-5 sm:px-6 sm:py-8 lg:px-8">
        ...
        <section className="mt-14 sm:mt-16"><div className="mb-6"><span className="text-[10px] font-black uppercase tracking-[0.2em] text-[#B76E79]">Complete your routine</span><h2 className="mt-2 text-2xl font-black text-[#1a1a1a] sm:text-3xl">You may also like</h2></div><ProductGrid products={related || []} /></section>
      </div>
    </div>
  )
}
