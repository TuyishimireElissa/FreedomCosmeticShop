...
export async function POST(req: NextRequest) {
  try {
    ...
    const { orderId, phone, network, language, orderAccessToken } = parsed.data
    ...
    if (!tokenValid && !accountOwnsOrder) return NextResponse.json({ error: 'FORBIDDEN_ORDER' }, { status: 403, headers })
    ...
    const providerConfigured = features.realPayments && Boolean(process.env.PAYPACK_CLIENT_ID && process.env.PAYPACK_CLIENT_SECRET)
