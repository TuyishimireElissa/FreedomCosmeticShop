...
export async function POST(request: NextRequest) {
  try {
    ...
    const { orderId, phone, orderAccessToken } = body as { orderId?: string; phone?: string; orderAccessToken?: string }
    if (!orderId || !phone || !orderAccessToken) return NextResponse.json({ success: false, error: 'orderId, phone, and order access token are required' }, { status: 400 })
    ...
    const proxyRequest = new NextRequest(request.url, { method: 'POST', headers, body: JSON.stringify({ orderId, phone: normalized, network: 'MTN', orderAccessToken }) })
