...
import { PUBLIC_PRODUCT_CARD_SELECT, PUBLIC_PRODUCT_SELECT, getRealUnitSales, serializePublicProduct, serializePublicProductCard } from '@/lib/public-product'
...
export async function GET(_request: Request, { params }: { params: Promise<{ slug: string }> }) {
  try {
    ...
    const related = relatedRows.map((item) => serializePublicProductCard(item, sales.get(item.id) || 0))
