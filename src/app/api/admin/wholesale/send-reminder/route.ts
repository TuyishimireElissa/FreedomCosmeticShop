...
export async function POST(req: Request) {
  try {
    const adminUser = await requirePermission(PERMISSIONS.WHOLESALE_UPDATE)
    ...
    const momoNumber = BUSINESS.invoice.momoPaymentNumber
    if (!momoNumber || momoNumber.includes('TODO:')) {
      return NextResponse.json({ error: 'Verified wholesale MoMo payment number is not configured' }, { status: 409 })
    }
    ...
    await db.notification.create({
      data: {
        ...
        body:
          type === "due"
            ? `Your invoice ${invoice?.invoiceNumber || ""} of ${invoice?.totalAmount || outstanding} RWF is due. Please pay to MTN MoMo: ${momoNumber}`
