...
export async function PATCH(request:Request,{params}:{params:{id:string}}){
  try{
    ...
    if(params.id===actor.id&&(input.isActive===false||input.role))return NextResponse.json({success:false,error:'You cannot deactivate or change your own role'},{status:400})
    const existing=await prisma.user.findFirst({where:{id:params.id,role:{in:['ADMIN','SUPER_ADMIN','STAFF','MANAGER']}},include:{staffProfile:true}})
    ...
    await logActivity({userId:actor.id,userName:actor.name,userRole:actor.role,action:'STAFF_UPDATED',entityType:'STAFF',entityId:params.id,description:`Updated ${existing.name}: ${changes.join(', ')||'metadata'}`,severity:sensitive?'critical':'warn',req:request})
    const user=await prisma.user.findUnique({where:{id:params.id},select:{id:true,name:true,phone:true,email:true,role:true,isDeleted:true,mfaEnabled:true,mustChangePassword:true,lastLoginAt:true,staffProfile:true}})
...
export async function DELETE(request:Request,{params}:{params:{id:string}}){
  try{
    ...
    if(params.id===actor.id)return NextResponse.json({success:false,error:'You cannot deactivate yourself'},{status:400})
    const target=await prisma.user.findFirst({where:{id:params.id,role:{in:['ADMIN','SUPER_ADMIN','STAFF','MANAGER']}}})
    ...
    await prisma.$transaction([
      prisma.user.update({where:{id:params.id},data:{isDeleted:true,deletedAt:new Date()}}),
      prisma.staffProfile.updateMany({where:{userId:params.id},data:{isActive:false}}),
      prisma.staffAccount.updateMany({where:{userId:params.id},data:{isActive:false,lastActivityAt:new Date()}}),
    ])
