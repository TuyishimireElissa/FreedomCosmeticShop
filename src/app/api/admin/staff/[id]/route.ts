...
export async function PATCH(request:Request,{params}:{params:Promise<{id:string}>}){
  try{
    ...
    await prisma.$transaction(async(tx)=>{
      const userData:{role?:string;passwordHash?:string;mustChangePassword?:boolean;passwordChangedAt?:Date;isDeleted?:boolean;sessionVersion?:{increment:number}}={}
      if(input.role&&input.role!==existing.role){userData.role=input.role;userData.sessionVersion={increment:1};changes.push(`role:${existing.role}->${input.role}`)}
      if(input.resetPassword){userData.passwordHash=await hashPassword(input.resetPassword);userData.mustChangePassword=true;userData.passwordChangedAt=new Date();userData.sessionVersion={increment:1};changes.push('temporary password reset')}
      if(typeof input.isActive==='boolean'){userData.isDeleted=!input.isActive;if(!input.isActive)userData.sessionVersion={increment:1};changes.push(`active:${input.isActive}`)}
      if(Object.keys(userData).length)await tx.user.update({where:{id:id},data:userData})
      if(userData.sessionVersion)await tx.authSession.updateMany({where:{userId:id,revokedAt:null},data:{revokedAt:new Date()}})
...
export async function DELETE(request:Request,{params}:{params:Promise<{id:string}>}){
  try{
    ...
    await prisma.$transaction([
      prisma.user.update({where:{id:id},data:{isDeleted:true,deletedAt:new Date(),sessionVersion:{increment:1}}}),
      prisma.authSession.updateMany({where:{userId:id,revokedAt:null},data:{revokedAt:new Date()}}),
