...
import { DESTRUCTIVE_OPERATIONS, PERMISSIONS, rateLimit, requireDestructiveOperation, requirePermission } from '@/lib/permissions'
...
export async function DELETE(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    await requireDestructiveOperation(DESTRUCTIVE_OPERATIONS.CONTENT_DELETE)
    const { id } = await params
