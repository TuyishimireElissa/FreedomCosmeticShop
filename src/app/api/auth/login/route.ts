...
export async function POST(req: Request) {
  try {
    ...
    let result
    try {
      result = await loginWithPassword({ identifier, password, rememberDevice: body.rememberDevice === true })
    ...
    if (
      ...
      const challengeToken = await MFAService.createLoginChallenge(result.user.id, body.rememberDevice === true)
      return NextResponse.json({
        success: true,
        mfaRequired: true,
        challengeToken,
    ...
    const mobile = req.headers.get('x-client-platform') === 'mobile'
    const res = NextResponse.json({
      user: result.user,
      message: "Login successful",
      ...(mobile ? { accessToken: result.accessToken, refreshToken: result.refreshToken } : {}),
    }, { headers: { 'Cache-Control': 'private, no-store, max-age=0' } })
    return setAuthCookies(res, result.accessToken, result.refreshToken, result.rememberDevice)
  } catch (error) {
