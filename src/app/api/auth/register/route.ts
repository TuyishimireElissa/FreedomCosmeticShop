...
export async function POST(req: Request) {
  try {
    ...
    if (!smsConfiguration.configured) {
      ...
      const response = NextResponse.json({ success: true, verificationRequired: false, user: result.user, message: 'Registration successful.', ...(mobile ? { accessToken: result.accessToken, refreshToken: result.refreshToken } : {}) }, { status: 201, headers: { 'Cache-Control': 'private, no-store, max-age=0' } })
