...
export async function POST(req: Request) {
  try {
    ...
    if (!data.isWholesale && data.paymentMethod !== 'CREDIT') {
      return NextResponse.json({ error: 'Retail checkout has moved to /api/orders/create' }, { status: 410 })
    }
    ...
    const orderAccessToken = await signOrderAccessToken(order.id)
    return NextResponse.json({ order: { ...order, orderAccessToken }, orderAccessToken }, { status: 201 })
  } catch (error) {
