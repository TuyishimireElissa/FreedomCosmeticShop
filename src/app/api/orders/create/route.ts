...
export async function POST(request: Request) {
  try {
    ...
    const publicOrder = { ...order, orderAccessToken }
    return NextResponse.json({ success: true, data: { order: publicOrder, confirmationDelivery, orderAccessToken }, order: publicOrder, confirmationDelivery, orderAccessToken }, { status: 201 })
  } catch (error) {
