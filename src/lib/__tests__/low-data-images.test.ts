...
describe('low-data responsive images', () => {
  it('defines Rwanda-mobile widths and eco quality', () => {
    expect(IMAGE_SIZES.card).toEqual({ lowData: 240, mobile: 300, desktop: 500 })
    expect(IMAGE_SIZES.detail).toEqual({ lowData: 480, mobile: 600, desktop: 800 })
    expect(IMAGE_SIZES.hero.desktop).toBe(1280)
  ...
  it('returns smaller card candidates in low-data mode', () => {
    ...
    expect(low.srcSet).toContain('300w')
    expect(low.srcSet).not.toContain('500w')
    expect(low.src).toContain('q_auto:eco')

    const normal = getResponsiveSrcSet('freedomcosmeticshop/products/test', { context: 'card' })
    expect(normal.srcSet).toContain('300w')
    expect(normal.srcSet).toContain('500w')
  ...
  it('uses direct reliable product URLs while retaining lazy loading and responsive helpers elsewhere', () => {
