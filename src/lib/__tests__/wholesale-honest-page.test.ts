...
const wholesaleConfigSource = read('src/lib/wholesale-config.ts')
...
describe('honest wholesale page', () => {
  ...
  it('uses only deployment-configured WhatsApp contacts without inventing hours', () => {
    expect(WHOLESALE_CONFIG.contacts).toEqual([])
    expect(wholesaleConfigSource).toContain('process.env.NEXT_PUBLIC_WHATSAPP')
    expect(wholesaleConfigSource).not.toMatch(/2507\d{8}/)
