...
describe('final comprehensive audit regressions', () => {
  ...
  it('binds Mobile Money initiation to the order and rate limits attempts', () => {
    ...
    expect(momo).toContain('verifyOrderAccessToken(orderAccessToken, order.id)')
    expect(momo).toContain('!tokenValid && !accountOwnsOrder')
  ...
  it('fails closed when production payment processing is disabled', () => {
    expect(momo).toContain("process.env.NODE_ENV !== 'production' && !providerConfigured")
