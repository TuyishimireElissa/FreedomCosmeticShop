...
describe('patched Next.js framework baseline', () => {
  it('pins the tested patched framework and matching ESLint config', () => {
    expect(packageJson.dependencies?.next).toBe('15.5.22')
    expect(packageJson.devDependencies?.['eslint-config-next']).toBe('15.5.22')
    expect(packageJson.overrides?.next?.postcss).toBe('8.5.18')
