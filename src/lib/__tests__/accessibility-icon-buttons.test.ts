...
const productCard = read('src/components/products/ProductGrid.tsx')
...
describe('accessible icon actions', () => {
  ...
  it('migrates search, wishlist, product, pagination, quiz, and wholesale icon actions', () => {
    ...
    expect(productCard).toContain('aria-pressed={wishlisted.has(product.id)}')
    ...
    expect(products).toContain('aria-controls="product-results"')
    expect(products).toContain('min-h-12')
