...
export class MFAService {
  ...
  static async createLoginChallenge(userId: string, rememberDevice = false): Promise<string> {
    ...
    return new SignJWT({ purpose: 'admin-mfa', userId, verificationId: verification.id, nonce, rememberDevice })
  ...
  static async verifyLoginChallenge(challengeToken: string, code: string) {
    ...
    return marked.count === 1 ? { userId: payload.userId, rememberDevice: payload.rememberDevice === true } : null
  }
}
